...
- **Done instead:** a `downloadQuality` preference (`ORIGINAL` — the default and the plan's behaviour — plus `HIGH`/`MEDIUM`/`LOW`) chooses between the original file and a server-side transcode requested as `/Videos/{id}/stream.mp4?static=false` with explicit H.264/AAC/mp4 parameters. `ORIGINAL` leaves every existing byte of the pipeline alone.

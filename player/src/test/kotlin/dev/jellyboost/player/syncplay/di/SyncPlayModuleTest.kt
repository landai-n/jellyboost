...
class SyncPlayModuleTest {
     ...
     * buffer of a half-open connection for minutes, so without protocol pings a NAT timeout leaves
     * the socket reading `Connected` for ever and the reconnect loop never runs.
     */
    @Test
    ...
    /**
     * The scope must run its coroutines one at a time: the controller's and the scheduler's
     * session bookkeeping are plain unguarded fields, and serial execution with a

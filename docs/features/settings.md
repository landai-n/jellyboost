...
| Playback | *Skip intro* and *Skip outro* — three-way pickers over `SegmentSkipMode` (`OFF` / `SHOW_BUTTON` / `AUTO_SKIP`); *Picture-in-picture* — enter a floating window when leaving the app mid-playback; *Text size* and *Background* — how large the app draws a subtitle and what it draws behind it (Follow the device / Small / Normal / Large / Larger, and Follow the device / None / Translucent / Solid), both defaulting to the device's own caption style and both reaching the playback already on screen; *Styled ASS subtitles* — libass rendering for ASS/SSA, experimental and default-off, read once per player build, and the one format the two settings above do not touch. See [`playback.md`](playback.md). |

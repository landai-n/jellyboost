...
/**
 * These tests run on the JVM with no shadowed Android framework, so `ConnectivityManager`,
 * `Network` and `NetworkCapabilities` are reached as MockK mocks and the registered
 * `NetworkCallback` is captured and driven directly.
 *
 * The seam being pinned: "metered" and "has network" are read off **one** registration, and an
 * offline device reports *not* metered — a guarantee `ConnectivityMonitor.isMetered` states in
 * prose, which nothing but a test can hold.
 */

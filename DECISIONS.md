...
- **Done instead / done:** the doctrine, with three deliberate departures from the canvas's own numbers and one from its scope, each recorded below.
  ...
  3. **The ghost pill's edge keeps white@40%, not the canvas's white@22%.** On `OverMedia.GlassFill` the canvas value measures 2.04:1 and a ghost pill's edge is its whole boundary (WCAG 1.4.11 asks 3:1); 40% — the dark scheme's own `GhostBorder` — is 3.56:1. Same precedent as `a9a4a9d0`, where three of the canvas's light hexes moved for a pinned ratio.

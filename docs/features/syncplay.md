...
|---|---|
| `SyncPlayController` | The coordinator. `@Singleton` with its own supervisor scope, **not** a ViewModel: membership outlives the player screen and survives backgrounding. Owns the state machine (`Idle / Joining / InGroup / Rejoining`), every user intent, the join handshake, connection-loss detection, teardown and `launchRequests`. A session's mutable state lives in one `GroupSessionState` box, so ending a session is a single assignment (teardown installs a fresh box; a stand-down installs `carriedAcrossStandDown`, which names what survives). The confinement contract — everything on the single-threaded `@SyncPlayScope` — is documented once, at the top of the class. |
| `SyncPlayRejoinPolicy` | The rejoin state machine, extracted from the controller (audit H5, 2026-08-07): the remembered target, the loss confirmation, the removal-after-trouble decision, the spaced attempt loop and the foreground re-check. Drives membership transitions through a `SessionDriver` the controller implements under its own membership lock — the same `performJoin`/`standDown` entry points every other path uses. |
| `SyncPlayRecoveryNets` | The two B3 timer safety nets (self-sync and pause), their two-window elicit-then-fallback shape, and the coarse `groupPlayingAnchor` the local fallback measures from — extracted from the controller (audit H5). A net's job owns its handle until its work is done (identity-guarded, the scheduler's SP-01 pattern), so a disarm always reaches a body suspended mid-hop (audit CPX-4). |
| `ReconcileAction.kt` | `decideReconcile(queue, loadedSlot, hostAttached, snapshot)` — the pure four-outcome decision (`None / RequestLaunch / Adopt / Load`) behind the controller's `reconcile`, carrying the slot-not-item identity and the adoption-only-before-first-load rule (audit CPX-15). |
...
| `socket/SyncPlaySocket` + `socket/OkHttpSyncPlaySocket` | Our own OkHttp websocket to `/socket`, lossless by construction: frames land in an unbounded channel before any decoding, then route through the SDK's `ApiSerializer` and the DTO mapping. Exists because the SDK's transport conflates (`SocketConnection.state` is a `StateFlow` carrying messages — DECISIONS.md 2026-07-31); `SdkSyncPlaySocket` stays in the tree, unbound, as the reference of that defect. Collected only while in a group — collecting opens the connection; reconnect (1→2→4→10 s, reset on success) and keep-alive replies are ours now. |
...
(`SELF_SYNC_TIMEOUT_MS`). It exists for one observed failure: after an automatic queue advance the
handshake completed, the group's state update said `Playing`, and no unpause ever arrived — leaving
the member at 0:00 under the WAITING overlay, unrecoverable by a group `Unpause` because the group
already *was* playing.
...
arms a second 3 s timer (`PAUSE_NET_TIMEOUT_MS`).

### Both nets, in two stages: ask the server first, guess only if it does not answer

Neither net acts locally when its first window expires. The server has a protocol-native recovery for
exactly this case and it is strictly better than anything this client can infer:

| server-side | what it does |
|---|---|
| `PausedGroupState.HandleRequest(PauseGroupRequest)`, `prevState == Paused` (`PausedGroupState.cs`:88-93) | *"Client got lost, sending current state"* — re-sends the authoritative `Pause` (exact `When`, exact `PositionTicks`) to `CurrentSession` only |
| `PlayingGroupState.HandleRequest(UnpauseGroupRequest)`, `prevState == Playing` (`PlayingGroupState.cs`:80-86) | the same for `Unpause` |

A request for the state the group is *already in* is therefore not a state change: it is how a member
that missed a `SendCommand` asks for it again, privately. So:

1. **Elicit** (first window, 3 s). The self-sync net sends `requestUnpause()`, the pause net
   `requestPause()` — through the ordinary `request { }` helper, fire-and-forget with a logged
   failure — and re-arms itself once for `COMMAND_REPEAT_TIMEOUT_MS` (2 s). The scheduler applies a
   re-sent command that never applied locally (its "applied exactly once" guard remembers only what
   actually reached the player), so the repeat lands and the ordinary command path — anchor, phase,
   drift monitor — does the rest.
2. **Local fallback** (second window). Only if that expires with still nothing applied does the
   controller act by itself, exactly as it always did.

Why this order: the local fallback's "group's inferred position" comes from the queue's
`startPositionTicks`/`lastUpdate`, which goes stale the moment the group pauses and resumes without
publishing a queue. On device that landed the self-sync seconds off and the desync compounded across
cycles (STATUS.md, logcat run 3). The elicited command carries the group's own timeline instead.

Two guards keep it from looping. **One elicit per episode**: the stage is carried by the re-armed job
(`NetStage.Elicit` → `NetStage.Fallback`), so the second window never asks again; a fresh
`StateChanged` or `ready` starts a new episode with its own single ask. And **the ask goes out only
where the fallback would have acted**: the group's state must still be the one the net was armed for
(an `UnpauseGroupRequest` to a group that has moved to WAITING would be this member starting
everyone, not a repeat), and this member must actually be out of step — phase not `Playing` for the
self-sync net, player actually running for the pause net.
...
| disarmed by (both stages) | any applied command, `StateChanged(Paused/Waiting/Idle)`, teardown | any applied command, `StateChanged(Playing)`, teardown |
| stage 1 (3 s) | `UnpauseGroupRequest`, if the phase is not already `Playing` | `PauseGroupRequest`, if the player is actually running |
| stage 2 (+2 s) | seek to the group's inferred position, `play`, then hand over to the drift monitor | `pause`, and only if the player is actually running |
| needs a host | **yes** for stage 2 — starting a detached player on a guess would be sound from nowhere | **no** — pausing a detached player the group has paused is right |
| reports | nothing — a request is not a `ready`, and another `ready` is what the B1 storm was made of | nothing |

The local stage is a deliberate, bounded exception to key decision 11; the elicit stage is not an
exception at all — it is a request to the server, and the player still moves only on a command.
...
The self-sync fallback's "group's inferred position" is `startPositionTicks` measured from the queue's
own `lastUpdate`, **not** from now: that position was true when the queue was published, and pairing
it with the current instant is what left a browser-initiated resume seconds short and then had the
drift monitor defend the short timeline (`syncplay-bugreport.md`). It is still only ever a guess,
which is why the elicit stage above now runs first.
...
| File | What it pins |
|---|---|
| `SyncPlayControllerTestBase` | The shared harness the three coordinator suites drive the real controller (and its real collaborators, wired to fakes) with — the extractions are deliberately covered through the controller's public surface. |
| `SyncPlayControllerTest` (48) | Join handshake, WAITING (overlay *and* pause), intents → API calls with **zero** local playback calls, `GroupGone` / `AccessDenied` / `Left` → Idle, no pause on a transient flap, a connectivity blip that keeps the group and re-negotiates, ready owed vs. silence, sign-out teardown, ignoreWait on detach, a detached player the group still reaches (phase stays `Playing`, a later command still applies), buffering reports carrying the player's real `isPlaying` while a `ready` parks a running player and reports `false` (and touches an already-stopped one not at all), the clock sampled before the join call (and a failed sample not blocking it), queue reconciliation, an adopted item that reports buffering and owes its `ready` (answered by the player, or by the 1.5 s fallback when it never re-buffers), unopenable slots skipped once, and an immediate ping on foreground while still in a group. |
| `SyncPlayRejoinPolicyTest` (21) | Connection loss and auto-rejoin, through the controller: a confirmed loss → paused + rejoin (and the ending message only once the attempts fail), connectivity loss vs. grace, ping-failure streak, a blip-then-`NotInGroup` and a `403` both taken back (join re-run, handshake re-entered, player never started, one "Rejoined" message), membership falling and rising for the local-session re-mint, a dissolved group asked after exactly once, exhaustion at exactly 3 attempts 2 s apart then one message and silence, aborts mid-rejoin from leave and sign-out, a removal over a healthy connection obeyed rather than undone, the exits that must never rejoin, and the foreground re-check family (asked again and got back, gone-in-the-meantime forgotten silently, a failed re-check silent/non-looping/retryable, the 10-minute window, deliberate exits never taken back). |
| `SyncPlayRecoveryNetsTest` (14) | The self-sync net and the pause net in both stages (a playing/paused group with no command is asked to repeat itself and the repeat lands through the scheduler, an elicit that goes unanswered leaving the local fallback to fire one window later, no second ask from the fallback window, a state flip inside the second window standing both stages down, the group's own pause standing the whole net down, nothing asked for or done to a player already stopped, a paused group at join, a self-sync cancelled when the group stops playing, a self-sync measured from the queue's `lastUpdate` rather than from now, a resume anchored on the parked player rather than the stale queue) — plus the two CPX-4 race regressions: a disarm landing while a net is suspended mid-hop cancels it, no stale fire (verified to fail against the old self-nulling bodies). The WAITING hold over a lying phase stays in the controller suite. |
| `SyncPlayReconcilePlanTest` (7) | `decideReconcile`'s four outcomes, the adoption-only-before-first-load rule and the slot-not-item identity, as a pure function. |

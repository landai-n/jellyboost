...
/**
 * Carries its own status-bar inset: there is no `TopAppBar` to supply one and both call sites draw
 * at the very top of the window.
 *
 * @param showSelectAll only where "all" is a definite set — the library grid pages its content.
 */
@Composable
fun SelectionAppBar(
    count: Int,
    ...
    Row(
        modifier =
            modifier
                ...
                // A minimum, not a fixed height (see `GlassBottomNav`): a hard 60dp clipped the
                // count label at accessibility font scales.
        ...
        Text(
            ...
            // Polite live region so each tap's new count is announced, with the untruncated string:
            // the visible one ellipsizes to "4 sel…" at narrow widths.
            modifier =
                Modifier
                    .weight(1f)

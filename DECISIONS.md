...
- **Done instead / decisions worth recording:**
  ...
  7. **CPX-15**: `reconcile`'s decision becomes a pure `decideReconcile(queue, loadedSlot, hostAttached, snapshot) : ReconcileAction` (None/RequestLaunch/Adopt/Load) in `SyncPlayReconcilePlan.kt`, unit-tested across the four outcomes and the adoption-only-before-first-load rule; the controller applies the action, and `loadedPlaylistItemId` has one write site per outcome.

...
- **1. `artistName` on the download row.** Database v11 → v12, still an `@AutoMigration`; nullable, no default. The enqueuer writes `albumArtist` where it is non-blank and the credited `artists` otherwise (blankness tested **per operand**, or a whitespace `albumArtist` swallows the fallback and the `IS NULL` backfill guard re-derives that same `null` for good); the metadata refresh fills older rows through **two** SQL-guarded statements — the existing `WHERE itemType IS NULL` one, extended, plus a new `backfillArtist` guarded on `WHERE artistName IS NULL`. The second exists because the column is younger than the first guard: a row stamped between the two has an `itemType` and no artist, so the grouping statement can never reach it, and the `IS NULL` test is what stops it overwriting a value an enqueue wrote. `DownloadItem.artistLine` resolves column → cached item, the same order `kind` uses.
- **2. The queue takes the same kind sections as the *Downloaded* tab** (`QueueSection`, `showQueueKindHeaders`), in the same `SECTION_ORDER` and under the same `KindHeader`. **Nothing in the queue folds** — an active transfer must never be hidden behind a header, which is why `QueueSection` is a flat list rather than a `DownloadGroup`. The flat `queue` list is untouched: the stats, the bulk targets and the reorder arithmetic all index into it. Reorder therefore changed too — up/down now targets the **nearest same-kind neighbour**, since swapping with a neighbour of another kind would leave the row exactly where it was drawn and reorder another section instead; no same-kind neighbour in that direction is a no-op, not a clamped move. That neighbour reaches `DownloadRepository.move` as an **id** — see the correction entry of the same date, which is where the reasoning for that lives.
...
## 2026-08-28 — the queue move takes a target **id**, not an index (corrects the entry above)
- **Scope:** `data/downloads/DownloadRepository.kt` (the `move` contract), `data/downloads/impl/DownloadRepositoryImpl.kt`, `feature/downloads/DownloadsViewModel.kt`, `docs/features/downloads.md`; five new repository cases and two new ViewModel cases.
- **Plan said:** nothing about the reorder's transport; `docs/PLAN.md:76` says only "reorder". The entry above introduced same-kind reordering and claimed the neighbour's index in the flat queue "leaves the other sections' relative order intact".
- **The claim was false, and an adversarial review of the branch found it.** There are **two** queue orderings and they are different lists: `DownloadsUiState.queue` is `toQueue()` — every unfinished row, `ERROR` and `CANCELLED` included — while `DownloadRepositoryImpl.move` reinserted at that integer into `DownloadDao.pending()`, which filters to `QUEUED`/`DOWNLOADING`/`PAUSED` because that is what the engine picks work from. One failed row at or above the target makes the same integer name two different rows. Two verified traces: `[trackE ERROR, trackA, trackB]` → `moveUp(trackB)` reinserted trackB exactly where it already was and wrote nothing; `[trackA, movieE ERROR, movieM, trackB]` → `moveDown(trackA)` aimed at the wrong row entirely.
- **Done instead:** `move(itemId, targetItemId)`. The ViewModel names the same-kind neighbour by **id**; the repository resolves the insertion index against **its own** `pending()` snapshot. The id doubles as the identity guard the check-then-act rule asks for — a target that finished, failed or was cancelled between the tap and the read is absent from the snapshot, so there is no place left to take and nothing is written. That is a no-op rather than a failure: the row the user aimed at has already left the list they aimed at it in. `DownloadDao.pending()`'s status filter is deliberately **not** touched — the download engine reads it.
- **The invariant is now written to what the code holds:** the moved row ends up where the named row was, and the rows the renumber covers keep their relative order. It does **not** extend to rows outside that set — a failed row keeps its stale `queuePosition` while the pending rows are renumbered from zero, so its place relative to them can still shift. That behaviour predates these sections and is left alone here rather than widened into an engine-ordering change; it is named in `docs/features/downloads.md` instead of being papered over.
- **Four smaller findings from the same review, fixed in the same batch:** (a) blankness in the artist expression was tested *after* the elvis in **both** mirrored writers, so `albumArtist = "  "` swallowed a perfectly good `artists` list — and permanently, since `backfillArtist`'s `artistName IS NULL` guard re-derives the same `null` on every later pass; both now test per operand. (b) `DownloadItem.artistLine`'s cached-item fallback read only `item.artists`, dropping the `item.albumArtist` the mapper populates and inverting the writers' own order; it now mirrors them. (c) `downloadedGroup` emitted two different node shapes under one `DOWNLOADED_ROW` content type, so one reuse pool held both; artless rows get their own constant. (d) `DownloadGroupCache` had no pin for the newly header-drawn artist, so nothing would have caught a future cheap-signature "optimisation" stranding it.
- **Reason:** an index is only meaningful in the list it was taken from, and this one crossed a boundary between two projections of the same table. The fix is the house rule applied literally — check-then-act on shared state needs an identity guard — and it removes the coupling rather than compensating for it.
- **Tests:** none weakened. `DownloadRepositoryImplTest` — the renumber-from-zero and land-at-the-end cases rewritten onto the id contract (same assertions, same behaviour pinned), plus a failed row above the target no longer costing the move, a failed row between the movers not sending the mover to a stranger's place, and a vanished target writing nothing. `DownloadsViewModelTest` — the three existing move cases now assert the id, plus both review traces. `DownloadEnqueuerTest` / `DownloadedMetadataRefresherTest` — the missing blank-`albumArtist`-with-`artists` cell in each writer. `DownloadRowsTest` — album artist outranking track artists in the fallback, and an album-artist-only item still credited. `DownloadGroupCacheTest` — an artist arriving invalidating the memoised sections and reaching the header.

## 2026-08-28 — the adversarial review becomes a gate at the end of every feature/fix wave
- **Scope:** new `.claude/skills/adversarial-review/SKILL.md`; `CLAUDE.md` Workflow expectations. Process only — no code.
- **Plan said:** nothing. The plan's quality gates are the mechanical ones (`/verify`'s five Gradle tasks, the guardrail scripts, the instrumented suite at DoD); review of a finished wave was ad hoc.
- **Done instead:** the multi-agent adversarial review that has now run twice — parallel reviewer lenses over the wave's diff, every finding verified by a refute-biased skeptic, only confirmed findings block — is a named gate, run once per feature/fix wave before its branch merges (or before the final checkpoint on main). The two runs to date confirmed 12 findings with 0 refuted against trees `/verify` had passed green, including an index crossing two projections of the queue table and a freshness key that a common write path never bumps.
- **No duplicated checks, by construction:** the skill carries an ownership table — formatting/style to ktlint, thresholds to detekt, regressions to the unit suite, static a11y to `:app:lintDebug`, hazard patterns/identifiers/docs/redaction/scaffolding/entry point to the six scripts, comment voice to the pre-commit hook, runtime semantics to the instrumented suite — and reviewer prompts must instruct agents to skip those classes. The gate owns only the audit-checklist classes no tool catches (cross-projection identity bugs, unfixed siblings, failure misclassification, lifetime mistakes, per-tick cost, untested doc claims). Its scope boundary against `/checkpoint` is also explicit: checkpoint is per sub-task, this is per wave.
- **Wiring caveat, stated rather than hidden:** the gate needs model access, so unlike every other gate it cannot be enforced by `pre-commit-gate.sh` or CI — `CLAUDE.md` is its wiring, and "merged without review" is to be treated like "committed without verify". If a hook-enforceable proxy ever becomes possible (e.g. a merge-commit marker), it should be added then.
- **Reason:** user direction 2026-08-28 ("Make the adversarial review a gate at the end of a feature/fix. Make sure we are not duplicating a check"), and the evidence above — the semantic defect classes have a 12-for-12 confirmation rate precisely where the mechanical gates are blind.

## 2026-08-28 — the series group header joins the music treatment: the season's poster and its name
- **Scope:** `data/downloads/model/DownloadItem.kt` (`seasonArtworkUrl`), `data/downloads/impl/DownloadRepositoryImpl.kt` (`SeasonArtworkCache`); `feature/downloads/{DownloadsUiState,DownloadsScreen}.kt`; `docs/features/downloads.md`. Extends the *downloads presentation wave* entry of the same date, whose point 3 deliberately left series headers alone.
- **Plan said:** nothing about what a group header draws; `docs/PLAN.md:76` specifies only a "*Downloaded* tab (grouped, sizes, delete)".
- **Done instead:** a **series** header now carries the same two slots the album header got — the **season poster** where the cover goes and the **season name** where the artist goes. Its episode rows keep their own artwork, which is where the parallel stops: an album's tracks all repeated the header's one cover, while an episode still differs per row. The name is the server's `seasonName`, never a `"Season $n"` composed here — the server has already localised it, and composing one would be a template string invisible to the `MissingTranslation` gate and owed a 69-locale pass; **no new strings**. A group is keyed by *series*, so it can hold several seasons: the distinct names are joined with `·` in season order (`parentIndexNumber`, unnumbered last) and the poster is the **first** of them, the season the unfolded list opens with — one thumbnail cannot honestly stand for four. A group no row's season reached keeps exactly its previous one-line header. The header draws both kinds' thumbnails at one square size, so a portrait poster is centre-cropped rather than giving the header a second geometry; `GroupHeader` itself is unchanged but for its KDoc.
- **Resolved at the data layer, not in Compose:** the season's poster is not on the episode (its `primaryImageUrl` is the still), so `DownloadRepositoryImpl` gains a `SeasonArtworkCache` — a second `DownloadMetadataCache` over the season parent rows `DownloadedMetadataRefresher` and `DownloadEnqueuer` already write, memoised on the same `cachedAt` key — and lands the answer on `DownloadItem.seasonArtworkUrl`. One extra narrow `getCacheKeys` per emission, no blob parse while nothing changed, and **no** probe at all for a list with no episode in it. A season row gone from the cache degrades to a header without a poster, never to a wrong one. A per-row lookup was refused for the reason the item join was: this flow re-reads on every throttled progress write, two to six times a second.
- **Reason:** user direction 2026-08-28, after they confirmed the music style — do the same for series, keep the per-row image because episodes genuinely differ, use the season image in the header, and put the season where the artist goes. The house rule that a UI heuristic must not stand in for a lossy join is why the poster is a field on the row rather than something Compose digs out of the joined item.
- **Tests:** none weakened. `DownloadsUiStateTest` — the season line and poster on a single-season group, the poster being the season's while the row keeps its own still, several seasons joined in season order with the first season's poster, an unnumbered season sorting last, one episode still missing its season not blanking the header, a cached-away season costing the poster but not the line, and a series no row's season reached keeping its one-line header; the pre-existing "a series header carries neither" case is kept and renamed to what it now pins — that neither the artist column nor the row's own artwork may reach a series header. `DownloadRepositoryImplTest` — the season join, a wiped season row answering `null`, a season written after the screen opened reaching rows already on it, and a list with no episode making no season query. `DownloadGroupCacheTest` — a season poster arriving invalidating the memoised sections and reaching the header, the sibling of the artist pin. The layout half (a square-cropped portrait poster in the header) is a Compose semantic no unit test here can see; `:feature:downloads` stays on `scripts/a11y-scaffolding-allowlist.json` and that debt is unchanged rather than newly incurred.
- **Correction (same date, from the adversarial review):** the rationale written above and in the KDoc — that the first season is "the one the unfolded list opens with" — was **false**. Rows inside a group sort by episode *title*, so the list can perfectly well open on a Season 2 episode; the multi-season test proves it. The behaviour is unchanged and still correct, but its reason is that the **lowest-numbered** season makes the choice canonical and stable, where "whichever the list opens with" would change the header's poster every time an episode is added or deleted. Corrected in `DownloadsUiState.kt`, `DownloadsUiStateTest.kt` and `docs/features/downloads.md`.

## 2026-08-28 — `items.revisedAt`: a freshness key of its own, because `cachedAt` is deliberately not one
- **Scope:** `core/database/entities/ItemEntity.kt` (`revisedAt`, `ItemCacheKey`), `DatabaseConstants.kt`, `JellyfinDatabase.kt`, `core/database/schemas/13.json`, `core/database/dao/ItemDao.kt`; `data/cache/ItemEntityMapper.kt`, `data/cache/BrowseCacheWriter.kt`, `data/downloads/DownloadedMetadataRefresher.kt`, `data/downloads/impl/DownloadRepositoryImpl.kt`.
- **Plan said:** nothing about cache-freshness keys; `docs/PLAN.md` specifies the Room cache but not how a decoded blob is memoised.
- **The bug an adversarial review of the series-header branch found.** `DownloadMetadataCache` (and so the new `SeasonArtworkCache`) keyed its memo on `cachedAt`, under a KDoc claiming that column "is bumped by every write to an `items` row and by nothing else". It is not: **two** writers deliberately carry the *old* `cachedAt` across an in-place rewrite — `DownloadedMetadataRefresher.store` and `BrowseCacheWriter.mergeRows` — precisely so a bulk repair or a browse write-through cannot reshuffle the offline "recently downloaded" order. That makes the commonest delivery path invisible to an open subscription: the enqueue caches a lean season row, the refresh later replaces it with the real artwork under the same `cachedAt`, and the screen keeps the blob it decoded first until the user leaves and comes back. The comment described code that did not exist, which is the audit's own recidivist class.
- **Done instead — fixed at the data layer, not in the memo:** a new `items.revisedAt` column, `NOT NULL` with a SQL default of `0`, stamped by **every** upsert path. That comes free rather than by discipline: all three writers build their rows through `ItemEntityMapper.toEntity`, which now sets `revisedAt` from the write time it is handed, and both preserving writers move `cachedAt` back with a `copy` that leaves `revisedAt` alone. `cachedAt` keeps its one job (the recency ordering) and stops pretending to a second. `getCacheKeys` selects both columns — still no `dto`, so the steady-state probe is as narrow as before — and the memo compares `revisedAt`. Database v12 → v13, still an `@AutoMigration`; the epoch default is the honest reading of a pre-v13 row and earlier than any real write, so the first rewrite after an upgrade invalidates whatever was held.
- **Deliberate over-invalidation:** `BrowseCacheWriter` advances `revisedAt` even where it preserves a download's rich blob, so an identical blob is occasionally re-decoded. That costs one parse which `distinctUntilChanged` then absorbs; the opposite error — a memo holding a blob that is gone — does not resolve itself at all.
- **Reason:** the house rule that a lossy column is fixed by adding the right column rather than by a heuristic over it, applied to a *sibling* bug the series header only exposed. The item cache had it too, so this heals both at once rather than patching the season path alone.
- **Tests:** none weakened. `SchemaMigrationTest` — v12 → v13 purely additive on `items`, `revisedAt` `NOT NULL` with default `0`, no table or index moved. `ItemEntityMapperTest` — the write time stamped onto `revisedAt`, which is what makes the one-place invariant hold. `DownloadedMetadataRefresherTest` — an in-place rewrite preserving `cachedAt` *and* advancing `revisedAt`, the two halves asserted on the same row (its `entity` stand-in mirrored the real mapper, which it had silently stopped doing). `BrowseCacheWriterTest` — the same pair on the write-through path. `DownloadRepositoryImplTest` — the season rewritten in place reaching an open screen (the case that was falsified), the existing "a rewritten item row is read again" case re-pointed at `revisedAt` with its `cachedAt` deliberately held still, plus the season join's steady-state cost: no re-parse across twenty progress writes, two key probes per emission and two row reads in total.

## 2026-08-28 — M14 scope extension: multi-server, collections, chapters, theming, gestures
- **Scope:** `docs/PLAN.md` (milestones), `STATUS.md`, `README.md`; upcoming work across `:core:ui`, `:core:network`, `:core:database`, `:data`, `:player`, `:feature:*`.
- **Plan said:** "NOT v1 (don't preclude): … multi-server UI …"; dark theme only ("Dark only by design; a light scheme is out of scope for v1", `Theme.kt`); collections/BoxSets unplanned; chapters fetched but deliberately not surfaced ("player chapter ticks" under *deliberately not built*, `docs/features/design-system.md`); player gestures limited to the M9 set (vertical brightness/volume swipes, double-tap seek).
- **Done instead:** added milestone **M14 — Breadth & theming**: multi-server + account switching, BoxSet collections, chapter UI in the player, light theme + Material You dynamic color, pinch-to-zoom and horizontal swipe-seek gestures, and a styled ASS/SSA subtitle-rendering spike.
- **Reason:** user direction 2026-08-28 — round out everyday client breadth now that the offline core and the three approved scope extensions (SyncPlay, Cast, Music) have all landed. Every M14 item was previously "don't preclude" or an explicitly recorded deferral, so none contradicts an architectural decision; the multi-server schema (`ServerEntity`/`ServerAddressEntity`) and the fetched-but-unsurfaced `ChapterInfo` were built for exactly this. Staying out of scope: Android TV, and any player-engine swap (the ASS spike decides an approach within ExoPlayer or records transcode burn-in as the accepted limit).

## 2026-08-28 — M14 track 4: Material You supersedes the pinned brand primary while the user has it on, and the splash stays dark-locked
- **Scope:** `core/ui/theme/Theme.kt` (`JellyfinTheme(themeMode, dynamicColor)`, `JellyfinLightColorScheme`, `LocalIsLightTheme`), `core/ui/theme/JellyfinColors.kt` (`JellyfinLightColors`), `core/ui/theme/PageInk.kt`, `GlassDefaults.kt`, `JellyfinGradients.kt`; `app/src/main/res/values/themes.xml` (unchanged, deliberately).
- **Plan said:** two things, from different places. docs/PLAN.md M14 track 4 asks for "a full light `ColorScheme` beside the dark one, optional Material You dynamic color on API 31+, theme preference (system/dark/light + dynamic toggle) in Settings", and says "every screen incl. player overlays and the download badges must resolve in both schemes". DECISIONS 2026-08-01 ("primary action buttons are white, colorScheme.primary stays #00A4DC") pins the brand primary against the M3 scheme, "for progress, selection, accents and links".
- **Done instead:** two narrow supersessions of that pin, and one exception to "every screen".
  (1) **Dynamic colour replaces the pinned primary while it is on.** `dynamicDark/LightColorScheme` derives *every* role from the wallpaper, primary included; there is no version of Material You that keeps `#00A4DC`. Rather than half-apply it — wallpaper backgrounds under a brand accent, which reads as a bug — the toggle hands the whole scheme over, and **defaults to off**, so the pinned primary remains what the app ships with and what every screenshot and doc describes. The 2026-08-01 pin stands for the default build; it is superseded only for a user who has explicitly asked for their wallpaper's palette. Below API 31 the toggle is not shown and the flag is ignored.
  (2) **The light scheme darkens the brand hues rather than reusing them.** `#00A4DC` measures 2.67:1 on the light page and `#AA5CC3` 3.89:1, both under the 4.5:1 WCAG 1.4.3 asks of the body text and links the pin exists to colour. `JellyfinLightColors.Primary`/`Secondary` keep the hue and saturation and take the lightness down to `#00769E` (4.79:1) and `#6B2F7F` (8.37:1). The pinned *identity* survives; the pinned *hex* cannot, on a ground it was never measured against.
  (3) **`themes.xml` and the splash theme stay dark-locked, permanently.** They draw before Compose exists and before DataStore can answer, so there is nothing to read a preference from; `Theme.Jellyboost.Starting` also force-sets `windowLightStatusBar=false` precisely to stop `Theme.SplashScreen`'s DayNight parent following the *system* independently of the app. A light-mode user therefore sees one dark splash frame before the app paints. This is the player-scrim precedent applied to the window layer, not a gap to close later.
- **Reason:** M14 track 4 asks for dynamic colour by name, and dynamic colour is all-or-nothing by construction; making that explicit here is cheaper than a future audit finding a doc saying the primary is pinned and a build where it is not. The light hues are forced by the same contrast arithmetic the dark palette's KDoc already documents.
- **Tests:** none weakened. `ContrastRatioTest` gains a light-scheme counterpart for every scheme-dependent pair it already asserted — palette roles, the ghost border, the focus ring, both progress tracks, the chip and pill inversions, the banner, and the glass chrome measured over the *darkest* frame rather than the brightest, because the light fill is additive and its worst case flips. Its mirror list is re-pointed at the new `pageInk`/scheme-derived declarations and gains the two over-media discs (`PlayerControls.OVER_MEDIA_DISC`, `NowPlayingScreen.OverMediaDisc`), which pin the claim that those two stay literal white in both schemes. `DataStoreAppPreferencesTest` covers `themeMode`/`dynamicColorEnabled` (default, round-trip, unknown stored name degrading to `SYSTEM`, emission per change).

## 2026-08-28 — M14 track 6: libass arrives as `ass-media` 0.5.1 **without** moving Media3 off 1.9.0
- **Scope:** `gradle/libs.versions.toml` (`libassAndroid`, `libass-media`, `androidx-media3-effect`), `player/build.gradle.kts`, `app/proguard-rules.pro`.
- **Plan said:** M14 track 6 — "styled subtitle rendering under ExoPlayer (e.g. a libass-backed renderer) … implementation only if it requires no player-engine swap". DECISIONS 2026-07-28 pins Media3 to whatever `org.jellyfin.media3:media3-ffmpeg-decoder` has a build for.
- **Done instead:** `io.github.peerless2012:ass-media:0.5.1` on the existing Media3 1.9.0, with `androidx.media3:media3-effect:1.9.0` declared to keep the graph on one version. **No Media3 bump.**
- **Reason — the bump was authorised and turned out to be unnecessary, then unavailable.** The research brief recorded ass-media 0.5.1 as "built against media3 1.11.0"; that is jellyfin-androidtv's own catalog, not the library's. `ass-media-0.5.1.pom` on Maven Central declares `media3-exoplayer`, `media3-ui` and `media3-effect` at **1.8.0**, so 1.9.0 satisfies it by ordinary Gradle resolution. Checked the other way too: `media3-ffmpeg-decoder`'s latest published version is still `1.9.0+1` (Maven Central metadata, `lastUpdated` 2025-12-29), so a 1.11.0 bump would have had no matching decoder build and the 2026-07-28 pin forbids it regardless. Smallest working combination wins, and it is the one already on disk.
- **Binary compatibility, verified rather than assumed:** every `androidx.media3.*` method reference in `ass-media-0.5.1.aar`'s bytecode (74 of them) was resolved against the media3 1.9.0 jars — 70 land directly on the named class and the remaining four (`Player.addListener`, `Player.getApplicationLooper`, `View.addView`, `View.getContext`) on supertypes. `AssRenderersFactory` implements `createSecondaryRenderer`, which 1.9.0's `RenderersFactory` has.
- **The one R8 rule this needs:** `AssMatroskaExtractor` reads `MatroskaExtractor.extractorOutput` and `MatroskaExtractor.subtitleSample` through `Class.getDeclaredField` **in a static initialiser**, and ass-media's `proguard.txt` is empty. Unkept, embedded-ASS MKV playback would fail with an `ExceptionInInitializerError` in release builds only. Both field names verified present in media3 1.9.0; the keep rule is in `app/proguard-rules.pro`. `ass-kt` (the native half) ships its own consumer rules covering all six JNI-visible types, so nothing is repeated for those.
- **Licensing:** bindings MIT; the natives statically linked into `libass.so` are ISC (libass), FTL/GPLv2 (FreeType), "Old MIT" (HarfBuzz), zlib (libunibreak, libpng), MIT (expat), BSD-like (fontconfig) and LGPL-2.1+ (FriBidi). All compatible with our GPL-3.0; no FFmpeg and nothing patent-encumbered. The AboutLibraries disclosure picks it up from the resolved graph automatically.
- **Size:** `ass-kt` ships four ABIs — arm64-v8a costs 3.04 MB `libass.so` + 17 KB `libasskt.so` (+ 1.25 MB `libc++_shared.so`, already in the APK via the ffmpeg decoder). That is **~+3 MB per ABI**, and because this build declares no `abiFilters` and no ABI split, ~+12 MB on the universal debug APK. Paid whether or not the preference is on: the `.so` is packaged, it is simply never loaded.

## 2026-08-28 — M14 track 6 verdict: styled ASS/SSA ships, default off, device validation owed
- **Scope:** `core/datastore` (`styledAssSubtitles`), `feature/settings` (switch + 69 locales), `player/session/AssSubtitleSupport.kt`, `player/session/AssMergeCompat.kt`, `ExoPlayerHandle`, `PlayerViewModel`, `PlayerScreen`, `gradle/libs.versions.toml` (`libass-kt`), `player/build.gradle.kts`, `docs/PLAN.md`, `docs/features/playback.md`, `docs/notes/m14-ass-libass-spike.md`.
- **Plan said:** M14 track 6 — "styled subtitle rendering under ExoPlayer … outcome recorded in DECISIONS, implementation only if it requires no player-engine swap". DoD: "spike verdict logged".
- **Done instead — the verdict, and one correction to the plan's own premise:**
  1. **Feasible, no engine swap.** `ass-media` is a pure Media3 extension: `AssRenderersFactory` *wraps* the existing `DefaultRenderersFactory` (and appends a `NoSampleRenderer`, leaving the ordinary `TextRenderer` in place), `AssSubtitleParserFactory` claims only `text/x-ssa` and delegates every other format to `DefaultSubtitleParserFactory`, and `AssSubtitleView` is a child of the `SubtitleView` already inside our `PlayerView`. ExoPlayer, the ffmpeg decoder extension, `MediaSession`/`PlaybackService`, `media3-cast` and the device profile are all untouched. The DoD's "no player-engine swap" condition is satisfied on the strictest reading.
  2. **Implemented as an experimental, default-off toggle**, matching jellyfin-androidtv's `libass_enabled` and *not* Wholphin's enabled-by-default. Rendered at `AssRenderType.OVERLAY_OPEN_GL`: full animation, HDR-safe (`EFFECTS_*` are not — androidx/media#723), rasterised off the UI thread.
  3. **`docs/PLAN.md`'s framing was imprecise and is corrected in this commit.** The blurb read "versus today's transcode burn-in". Burn-in is not the baseline: `DEFAULT_DIRECT_PLAY_ASS` has been `true` since M5, ASS/SSA is direct-played, and `Encode` only happens when the file is already transcoding for an unrelated reason. The real comparison is against Media3's `SsaParser`, which keeps alignment, two colours, font *size*, B/I/U/S and `\pos`/`\move`/`\an` and strips font family, karaoke, animation, clipping, rotation, margins and MKV font attachments (verified against `SsaStyle.java`; ExoPlayer#8435 open and low-priority since 2021, no SSA work in media3 1.9–1.11).
- **Reason:** doing nothing was not neutral — the built-in subset is frozen upstream — and the integration risk, not the library choice, is what the research flagged. Shipping it off-by-default puts the code in the tree where the owed device walk can exercise it, without changing a single install's behaviour until that walk passes.
- **Deliberately not touched:** `DeviceProfileBuilder.subtitleProfiles` and its `hlsTextSubtitles` branch (the 2026-08-21 subtitle-drift fix depends on it — ASS on the transcode path stays VTT, so libass buys nothing there and could only reopen the drift bug); `CastPlayerHandle` (a separate `PlayerHandle` with no local surface — libass is unreachable from it, and the receiver renders its own subtitles); `TrackSelectionController` and the `external:<index>` id scheme.
- **Id collision, resolved by reading the module rather than the README.** `ass-media`'s README asks for numeric `SubtitleConfiguration` ids "128 or bigger". The actual constraint is its `availableTracks` map, keyed on `Format.id` across embedded (numeric MKV track numbers) *and* side-loaded tracks; `external:<index>` cannot collide with an MKV track number, so the scheme is kept and both `TrackSelectionControllerTest` and `ExoMediaSourceFactoryTest` pass unweakened and unedited.
- **A real ceiling found in the module, and guarded.** `updateTrack()` matches `selectedFormat.id.substringAfter(":")` against the key it stored at parse time, i.e. it strips exactly **one** merge prefix. `MergingMediaPeriod` adds one prefix per merge, and an item with audio sidecars *and* side-loaded subtitles is merged twice (`0:1:external:2`) — no match, and because a non-`CUES` render type hands the `TextRenderer` an `AssNoOpSubtitleParser`, the result would be **no subtitle at all**, not a fallback. `styledAssSurvivesMerge` detects that shape and builds those items with the plain factory: unstyled, but on screen. That combination is a downloaded multi-audio item with subtitle sidecars, which is exactly the anime case the feature targets, so the guard is not theoretical.
- **Failure classification:** a libass load failure is **permanent** for the process (missing ABI, stripped release, refused GL stack) and has a working alternative, so it degrades to `SsaParser` with a warning and never fails the playback; `AssHandler.ass` is touched at creation to force `System.loadLibrary` there rather than at the first subtitle sample. `runCatchingUnlessCancelled` throughout.
- **Size:** ~+3 MB per ABI, unchanged from the dependency entry above and paid whether or not the switch is on — the `.so` is packaged, it is simply never loaded.
- **Still owed, and the feature stays experimental until it is walked** (the seven checks are spelled out in `docs/notes/m14-ass-libass-spike.md`): (1) styled ASS **visible at all** on the production `PlayerView`/`SurfaceView` path at `OVERLAY_OPEN_GL` — the Wholphin #1049 revert was exactly this failure, and it is pass/fail for the feature; (2) embedded MKV ASS with **attached fonts**, animation and karaoke; (3) an external `.ass` sidecar from the download path, with the `external:<index>` mapping intact and the double-merge guard confirmed; (4) **sign-timing offset** measured against server burn-in, quantifying libass-android #71 / androidx/media#2289 (closed upstream as *wont fix: infeasible*) on this hardware; (5) no regression on the **HDR/HEVC** path against the tablet's decoder limits (no Main 10 HEVC, hardware decode capped at 2560x1440); (6) **stability** under surface resize, orientation change, PiP enter/exit, mid-playback track switching, plus a memory check with `maxRenderPixels`; (7) **cast-path inertness** with the switch on, in both transfer directions. If (1) or (4) fails, the honest outcome is to downgrade to *do not implement* and record that here.
- **Tests:** none weakened. New: `AssMergeCompatTest` (four cases pinning the double-merge guard, including the shape that would otherwise render nothing); `DataStoreAppPreferencesTest` gains default / round-trip / emission for `styledAssSubtitles`; `SettingsViewModelTest` gains the write-through and the default-off projection. `PlayerViewModelFixture` and `PlayerViewModelCastTest` construct an `AssSubtitleSupport` whose preference is off, so no test ever asks for a native library.
## 2026-08-28 — M14 track 1: offline layer is single-identity; "schema-ready" claim corrected, prerequisite workstream enlarged
- **Scope:** `docs/PLAN.md` (M14 track 1 wording), `docs/notes/m14-multiserver-design-brief.md` (engineering context, surfaces list, backend workstream); the future backend work touches `core/database` (`UserDataDao`, `DownloadDao`, `DownloadEntity`, `ItemEntity`, `UserDataEntity`, `LibraryViewEntity`), `data/userdata` (`UserDataSyncer`), `data/downloads` (`DownloadedMetadataRefresher`, queue), `core/network` (`SessionRepository`, `SessionGate`).
- **Plan said:** "surface the schema-ready `ServerEntity`/`ServerAddressEntity` in UI … tokens stay per-user in `SecureCredentialStore`; Room user data is already keyed by userId, so watch state survives switching."
- **Done instead:** docs-only correction ahead of any implementation. A code trace (2026-08-28) showed the claim holds for *reads* only: the schema is multi-*address* ready, not multi-server scoped. `UserDataDao.getPendingSync()` is `WHERE toBeSynced = 1` with no identity filter, so a switch drains one identity's pending offline watch-state rows against the other identity's server — and `UserDataSyncer`'s 404-abandon path then silently destroys the only copy of the change, which alone fails the M14 DoD "per-account watch state intact". Below that: `downloads`/`items`/`library_views` carry no `serverId` while Jellyfin GUIDs are per-server (primary-key collision possible); `DownloadDao` has no ownership filters ("delete downloads too" on sign-out deletes every identity's rows); queued downloads rebuild URLs from the live ApiClient, so an unguarded resume fetches from the wrong server; `DownloadedMetadataRefresher` refreshes every row against the current server; and a switch that bypasses `signOut()` skips the browse-cache wipe (HYG-2 would reopen). PLAN.md's track-1 wording now names the offline-scoping prerequisite, and the design brief's "After the canvas is saved" step (1) was expanded from "multi-slot `SecureCredentialStore` + list DAOs" into the full workstream (serverId columns, keyed sync drain, queue gating, refresher filter, switch-path hygiene, scoped delete, identity-verifying `SessionGate`). The brief also gained surface (6): the Downloads tab's shared-device union view keeps foreign-server rows visible and playable, with a provenance label only once a second server's downloads exist, offline-style detail routing, and "Waiting for <server>" held state for unfinished foreign rows.
- **Reason:** governance rule 4-adjacent — a plan claim describing guarantees the code does not provide is the doc-without-test recidivist class at plan scale; correcting it before the switching UI is designed is cheaper than after. The shared-device stance for downloads survives; only its backend footing was understated.
- **Tests:** none touched (docs-only). The workstream carries its own pins when it lands: "pending sync rows for identity A are neither drained nor abandoned while identity B is active" and "a queued download for server A does not run against server B"; DoD extended to a dirty offline sync queue surviving a switch.

## 2026-08-28 — M14 track 4 (review fix): the saved canvas's light palette is adopted, with three values moved for a pinned ratio
- **Scope:** `core/ui/theme/JellyfinColors.kt` (`JellyfinLightColors`), `core/ui/theme/PageInk.kt`, `GlassDefaults.kt`, `core/ui/component/JellyfinButtons.kt`, `JellyfinTextField.kt`, `PillChip.kt`, `ErrorBanner.kt`, `feature/detail/ItemDetailHeader.kt`, `feature/downloads/DownloadRows.kt`, `feature/downloads/DownloadsScreen.kt`, `core/ui/test/ContrastRatioTest.kt`, `design/_shared/tokens.css`, `design/foundations/colors-light.html`, `docs/features/theme.md`, `docs/features/design-system.md`.
- **Plan said:** CLAUDE.md's workflow rule — "strong UI changes / new UI go through Claude Design first … implementation follows the saved canvas". The saved M14 canvas (commit 306b7c6c, "light-theme foundations") specifies the light scheme.
- **Done instead:** the canvas palette is adopted wholesale, superseding the branch's implementation-derived one, **except** the three values that miss a ratio `ContrastRatioTest` pins. Those keep the canvas's hue and move only lightness or alpha, by the minimum the measurement asks for:
  1. **`primary` `#0089B8` → `#00769E`.** The canvas blue is 3.52:1 on its own `#EEF1F7` page and white on a `#0089B8` pill is 3.98:1 — both under the 4.5:1 WCAG 1.4.3 asks of body text, links and a filled button's label. `#00769E` is the *lightest* point on that same hue's ramp clearing both: 4.54:1 on the page, 5.14:1 on a card, 5.14:1 under white. (It is also what the branch shipped, arrived at independently on a different ground.)
  2. **`outline` `#D4DAE6` → `#788AB0`.** `outline` is M3's *meaningful* boundary role, which WCAG 1.4.11 asks 3:1 of — the obligation the dark `#6E6E6E` was remediated for. The canvas value is a 1.24:1 seam on the page and 1.40:1 on a card, which cannot carry it. Hue (220°) and saturation (26.5%) held, lightness 86.7% → 58%: 3.06:1 and 3.46:1.
  3. **`onSurfaceVariant` `#1F2330`@60% → @78%.** The binding ground is not the page (4.05:1 at 60%) but the light scheme's worst text case, `GlassDefaults.LightChromeFill` over the darkest frame — rgb(171,174,178) — where 60% reads 3.06:1. 78% is the first step clearing 4.5:1 there (4.53:1); on the page it is 7.09:1.
  Adopted unchanged: `background` `#EEF1F7`, `surface` `#FFFFFF`, `surfaceVariant` `#E3E8F2`, ink `#191B22`, `secondary` `#9A4DB4` (4.53:1 as drawn), `error` `#B3261E`, the backdrop scrim, and the light `.brand-glow` alphas (.16/.10).
- **Dependent alphas that had to follow the darker ink** (`#191B22` at a given alpha is lighter than `#101010` was, and the page is darker): `pageInk`'s light text alpha 0.60 → **0.65** (3.04:1 → 5.08:1 at the disabled pill, placeholder and bulk-action label) and its light component alpha 0.44 → **0.48** (2.72:1 → 3.04:1 at the focus ring and both progress tracks). `GlassDefaults.LightGhostBorder` (black@44%) still clears 3:1 at 3.18:1 and is unchanged.
- **The canvas's separate light `--image-placeholder` (`#DCE2EE` → `#EAEEF6`) was not adopted as literals.** `JellyfinGradients.ImagePlaceholder` is derived from `surfaceVariant`/`surface`, which is what lets it follow Material You's wallpaper palette; hardcoding a blue-grey pair would leave a wallpaper-tinted app with one un-tinted surface. Under the adopted palette it resolves to `#E3E8F2` → `#FFFFFF`, one step of the same cool ramp. `tokens.css` records the resolved values, per its own "source of truth is the Kotlin theme" header.
- **Also not reconciled (recorded so it does not read as a missed sibling):** `design/_shared/modern.css`'s light block carries a fuller glass layer than `GlassDefaults` does (`--glass-bg` white@62% vs `LightFill`'s 55%, `--elev-*` blue-tinted shadows, `--error-fg` `#A8241C`). That is a surface-layer question, not the palette this entry is about, and none of it is a contrast obligation the test pins.
- **Reason:** design-first governance — the canvas is the settled design and wins over an implementation-derived palette. Contrast is the one thing a canvas cannot settle, because it is arithmetic against grounds the canvas does not render (chrome over a black frame, glass wells, the pill inversion); where the two conflict, the ratio wins and the hue is preserved.
- **Supersedes:** the 2026-08-28 "M14 track 4" entry's clause (2), whose `#6B2F7F` secondary and `4.79:1`/`8.37:1` figures were measured against `#F6F7F8`. The *identity* argument in that clause stands unchanged; only the ground and two hexes move.
- **Tests:** none weakened. `ContrastRatioTest`'s light half is re-measured against the new palette (every `Floor` still passes on its own arithmetic); the light cases that stood in for `pageInk` tokens with a `Color.Black` literal now read `JellyfinLightColors.OnBackground`/`OnSurfaceVariant`, which is what the code actually draws; the `Exempt` records for the banner border (1.60 → 1.59) and the `KnownViolation` for the storage track (1.32 → 1.28) are re-recorded; and the light **disabled bulk-action label** gains the counterpart case it never had (5.23:1 at the new alpha, 3.09:1 at the old one).

## 2026-08-28 — M14 track 6 (review fix): the styled-ASS switch's copy is corrected rather than the player rebuilt
- **Scope:** `feature/settings/src/main/res/values*/strings.xml` (`settings_styled_ass_supporting`, 70 files), `player/session/AssSubtitleSupport.kt` (KDoc), `player/src/test/.../AssPreferenceStalenessTest.kt` (new), `docs/features/playback.md`, `docs/notes/m14-ass-libass-spike.md`.
- **Plan said:** the 2026-08-28 track-6 verdict entry records the contract as "read once per player build … a change in Settings reaches the next playback and not the one on screen", and the shipped switch said "applies to the next video you start."
- **Done instead — the copy is made true, not the code.** The contract's second half is only true when the player is *rebuilt* in between, and a music session deliberately prevents that: `MusicPlaybackController.relinquishToOther` hands the player over with `onPlayer { release() }` — the music *adapter*, not the player — because the claimant prepares on the same instance, and `ExoPlayerHandle.release()` additionally defers while `PlaybackService` is up. So toggling the switch with music loaded leaves the next video on the player built for the album, unstyled, while the switch reads on. Casting holds the instance the same way. The string now reads "applies to the next video you start **with nothing else playing**", in English and all 69 translations, and `playback.md`, the spike note and the `AssSubtitleSupport` KDoc carry the condition and the mechanism.
- **The rebuild was considered and rejected, with the blocker named.** Rebuilding at video-prepare time is not a local change: `PlaybackService` builds its `MediaSession` around the live `ExoPlayer` (through `SyncPlayAwareForwardingPlayer`), so a rebuild means re-pointing that session under a foreground notification, plus a second `AssHandler` alive across the swap and a `PlayerView` re-bind. `PlayerViewModel` reads `playerHandle.player` *after* `sessionController.open` returns, so the UI half would in fact be safe — but the session swap is a media-session change that only a device walk can clear, and the walk this feature already owes has not run. Paying that risk for a default-off experimental toggle, to close a window that only opens when a user changes a subtitle setting mid-album, is the wrong trade. If the owed walk passes and the feature comes off experimental, the rebuild is the right follow-up; the test below is what will tell whoever writes it which copy and docs must move with it.
- **Accepted staleness, stated plainly:** with a music or cast session alive, a change to `styledAssSubtitles` does not reach the next video; it reaches the first video started after the shared player is released.
- **Reason:** governance rule 4-adjacent and the audit's "every doc claim needs a test" — a shipped sentence promising behaviour the code does not provide is the recidivist class. Between correcting the sentence and rebuilding a media session for an experimental default-off feature, the sentence is what should move.
- **Tests:** none weakened. New `AssPreferenceStalenessTest` pins the three source facts the copy rests on — the preference is read in exactly one place and that place is `buildPlayer()`, `requirePlayer()` reuses an existing instance, and the music handover releases the adapter rather than the player. It fails the day someone adds the rebuild, which is when the copy has to change back. `MusicPlaybackControllerTest`'s handover case already pins the release-not-stopAndRelease behaviour from the music side. `validate_i18n.py` is green across 759 locale files.

## 2026-08-28 — M14 track 6 (device fix): libass is handed a fontconfig configuration, because the one compiled into `ass-kt` cannot exist on a device
- **Scope:** `player/session/AssFontConfig.kt` (new), `player/session/AssSubtitleSupport.kt`, `player/src/test/.../AssFontConfigTest.kt` (new), `player/src/test/.../PlayerViewModelFixture.kt`, `docs/notes/m14-ass-libass-spike.md`, `STATUS.md`.
- **Plan said:** the 2026-08-28 track-6 verdict entry records the integration as *wrapping* Media3 and nothing else — "`ass-media` is a pure Media3 extension … ExoPlayer, the ffmpeg decoder extension, `MediaSession`/`PlaybackService`, `media3-cast` and the device profile are all untouched" — and lists a seven-check device walk as owed. Nothing in it anticipates the app configuring the *native* library's environment.
- **Done instead — the first device run passed check (1) and produced a new finding, and the fix reaches outside the wrapping.** Styled ASS is drawn on our `PlayerView`/`SurfaceView` path at `OVERLAY_OPEN_GL` (the Wholphin #1049 failure does not reproduce), but **the words ran together**: glyphs, colours, outlines and positions correct, inter-word gaps absent.
  1. **Root cause, from the shipped binary.** `ass-kt` 0.5.1's `libass.so` carries its build-tree fontconfig paths verbatim — `.../lib_ass/.cxx/RelWithDebInfo/<hash>/<abi>/etc/fonts`, `.../share/fontconfig/conf.avail`, `.../var/cache/fontconfig`, all under the CI runner's home. None exists on Android, so fontconfig takes its *"No usable fontconfig configuration file found, using fallback"* path: a built-in document with `/system/fonts` and `/product/fonts` but **no `conf.d`**, and an absent cache directory.
  2. **Why that removes spaces specifically.** `AssKt.c` calls `ass_set_fonts(renderer, NULL, "sans-serif", ASS_FONTPROVIDER_FONTCONFIG, NULL, 1)`, and libass's fontconfig provider builds its whole fallback list once by running `FcFontSort` over a pattern whose only family is `sans-serif` (`ass_fontconfig.c: cache_fallbacks`). Without `conf.d` there is no alias to resolve it, so the sort is an arbitrary ordering of every font on the device and `get_fallback` serves each codepoint from the first entry covering it. Letters are covered only by real text fonts and therefore still render correctly; **U+0020 is covered by nearly every font installed**, icon and clock faces included, and those have a zero space advance. `default_font` is `NULL` in the same call, so there is no last-resort file underneath.
  3. **Fix.** `AssFontConfig` writes a `fonts.conf` into the app's `filesDir` and names it in `FONTCONFIG_FILE` through `android.system.Os.setenv`, from `AssSubtitleSupport.createHandler()` — before anything can load libass, because the variable is read once inside the `ass_set_fonts` call `AssHandler` makes while building its renderer. It carries the Android font directories, **the `sans-serif` alias** (`Roboto`, `Noto Sans`, `Droid Sans`, `DejaVu Sans`) which is the load-bearing part, and a writable `cachedir` under `cacheDir` so a session's first subtitle stops paying an uncached scan of `/system/fonts`.
- **Reason:** it is the smallest fix that addresses the cause, and it has shipping precedent for the same platform gap — ffmpeg-kit exposes `FFmpegKitConfig.setFontconfigConfigurationPath` precisely because Android ships no fontconfig configuration and a library assuming one must be handed one. It is recorded here because "the app sets a process environment variable and writes a file for a native dependency" is outside the wrapping the verdict entry describes, and would otherwise read as unexplained.
- **Rejected alternatives.** *A bundled fallback font* registered through `AssHandler.addFont`: unnecessary — `/system/fonts` is present and readable, only unreachable through a broken sort — and it would not fix the sort, since libass matches the embedded provider by a font's real family name and would still resolve `sans-serif` through fontconfig. *A library bump*: 0.5.1 is the latest release (2026-08-11), nothing since it touches fonts, and libass-android has no issue reporting this; a bump would have been a change with no fix in it. *Patching upstream*: worth doing, but it does not help an install today.
- **No sibling.** `sans-serif` is the only generic family libass ever asks fontconfig for — both as `family_default` and as the `cache_fallbacks` pattern — so `serif`/`monospace` aliases would be inert and are omitted. Attached MKV fonts are unaffected: they reach libass through `ass_add_font` and its embedded provider, which never consults fontconfig.
- **Failure classification:** permanent for the process and **not** fatal. If the file cannot be written or the variable cannot be set, libass keeps the platform's unusable defaults — the behaviour that shipped before this — so it is logged at warn and playback continues rather than losing styled subtitles altogether.
- **Tests:** none weakened. New `AssFontConfigTest` pins the document's font directories, the `sans-serif` alias *and its preference order*, the writable cache directory, the rewrite-only-on-change behaviour (so a cache fontconfig has already built survives), and — as a source check, since an `AssHandler` cannot be constructed off a device — that the environment is set before the handler that reads it. `PlayerViewModelFixture` gains a relaxed `Context`; its preference stays off, so nothing in the suite reaches either the file or the native library.
- **Still owed:** checks (2)–(7) of `docs/notes/m14-ass-libass-spike.md`, plus a spacing re-check on the same walk — real gaps between words, and a style whose family is absent now falling back to Roboto rather than to whatever sorted first.

## 2026-08-29 — M14 track 4: image territory is dark-scrimmed in both themes; the light page begins at the artwork's edge
- **Scope:** `core/ui/theme/OverMedia.kt` (new), `core/ui/theme/ChromeBackdrop.kt` (new), `core/ui/theme/JellyfinGradients.kt`, `core/ui/component/{JellyfinButtons,PillChip,BackdropHeader}.kt`, `feature/home/{HomeHero,HomeScreen}.kt`, `feature/detail/{ItemDetailHeader,ItemDetailScreen}.kt`, `app/{AppScaffold,AppActions}.kt`, `core/ui/src/test/.../ContrastRatioTest.kt`, `design/_shared/tokens.css`, `design/screens/{home-light,detail-landscape}.html`, `docs/features/theme.md`.
- **Canvas said:** the saved `screens/home-light.html` page now states the doctrine directly — *"On artwork the light-theme scrim (fading to #EEF1F7) milks out the picture and leaves the white glass chrome unreadable. Image territory stays dark-scrimmed / light-on-image in BOTH themes; the light surface begins at the hero's bottom edge."* It names the values: a `#0C0E14` ramp (0 → .30 at 42% → .78), white title, white@.78 eyebrow, white@.74 meta, a white@.32 certificate edge, `#0A0C12`@.42 glass with a white@.22 edge, and the **dark** halo strengths in both themes.
- **Plan said:** nothing at this resolution — `docs/PLAN.md`'s M14 track 4 is "light theme"; this is the canvas resolving what that means over artwork, and it supersedes yesterday's `ca962094`, which was a calibration of the *page-fade* approach this replaces.
- **Done instead / done:** the doctrine, with three deliberate departures from the canvas's own numbers and one from its scope, each recorded below.
  1. **`BackdropScrim`'s light branch is the pinned ink ramp**; the dark branch still ends on the real `colorScheme.background`, because there the page colour *is* the ink and that keeps a dynamic-colour dark page seamless. `WideHeroScrim`'s two branches collapse to one set of stops — only the colour ever differed. `heroHalo` drops its dimmed light pair: it draws on image territory, which has no light ground to stain.
  2. **`OverMedia` is a new token object, not `pageInk` with a second alpha.** It is the artwork half of the rule `PlayerControls` already states for video, and it is what `HomeHero`, the compact detail lockup, and the over-media pill/chip/badge variants read. `OverMedia.artworkDissolvesIntoPage` is the one *layout* bit it publishes: the wide hero's rail fade and its 48dp overlap are drawn only where the artwork reaches the page. The wide copy's bottom inset stays `HeroRailOverlap` in both schemes, so every calibrated height threshold in `HomeHero` is still measured against one number and `HomeSizingTest` is untouched.
  3. **The ghost pill's edge keeps white@40%, not the canvas's white@22%.** On `OverMedia.GlassFill` the canvas value measures 2.03:1 and a ghost pill's edge is its whole boundary (WCAG 1.4.11 asks 3:1); 40% — the dark scheme's own `GhostBorder` — is 3.54:1. Same precedent as `a9a4a9d0`, where three of the canvas's light hexes moved for a pinned ratio.
  4. **Chrome over artwork takes `#101010`@72%, not the canvas's `#0A0C12`@42%.** The 42% fill is specified for buttons inside a copy lockup, where the scrim is already under them (13.97:1 for white). Chrome floats over *unscrimmed* artwork — the hero's top corner — and there the same fill leaves white at 2.85:1 against 7.70:1 for the chrome tint. `OverMedia.ChromeFill` is therefore `GlassDefaults.DarkChromeFill`, which is the ratio `ContrastRatioTest` already pins.
  5. **The wide detail stage keeps a page-fading scrim** (`JellyfinGradients.StageScrim`, the eased light ramp `ca962094` calibrated) and its lockup keeps scheme ink. It is the app's one layout where the page's own content is drawn *across* the artwork's foot — the poster and the facts column both start inside the backdrop and finish below it — so there is no bottom edge for a light surface to begin at, and splitting the lockup's ink at the seam would read worse than either side of it. `design/screens/detail-landscape.html` is the canvas that shows this, and it still uses the page-fading token.
- **Reason:** the canvas is the design authority for what the light theme looks like, and its complaint is exact — a scrim that fades to `#EEF1F7` raises the artwork's luminance until nothing drawn on it has a ground. Departures 3 and 4 are the standing house rule that a pinned contrast ratio outranks a canvas hex; departure 5 is a layout fact the canvas's home page does not speak to.
- **Chrome's over-media state is threaded, not guessed.** The band and the action cluster are siblings of the nav host and cannot see the hero, and the hero is inside a `LazyColumn` whose scroll decides the answer — so `ChromeBackdrop` is published downward by `AppScaffold` and written by `HomeScreen` from a `derivedStateOf` over the list's offset against the hero's height, cleared on dispose. It is deliberately **not** a theme bit: the ground under the bars changes as the hero scrolls away, in both schemes.
- **Deliberately not changed:** `ItemDetailScreen`'s overlay nav circles. The canvas asks for dark glass on glass over artwork, but these keep `GlassDefaults.ChromeFill` — the *unscrimmed* top of a detail backdrop is where they sit, which this change does not touch (the new ramp is transparent there), both schemes' chrome tint is derived against the darkest frame **and** the page, and the detail screen's scroll state is not visible where the overlay is drawn, so pinning them dark would put a black disc on a light page the moment the page moved. `GlassTopNav` is left alone for the same reason plus one more: over the hero it now sits on the pinned dark band, which is the darkest-artwork case its light fill was derived against.
- **Tests:** none weakened. Ten cases added to `ContrastRatioTest`, all measured over a fully white backdrop: the lockup's title/eyebrow/meta at the scrim's 78% foot (9.89 / 6.78 / 6.29), the same title and meta at the wide wash's 72% mid stop (7.88 / 5.20), the over-media pill's label and edge on its glass (13.97 / 3.56), the chrome glyph (7.70), the primary pill's inversion (19.03), and the certificate's edge frozen as `Exempt` at 2.48 with its reason. Three `MIRRORED_DECLARATIONS` entries pin the hero's and the detail lockup's over-media *call sites*, as most of the player's already were — **corrected 2026-08-29:** "as the player's already are" was too strong. `UpNextCard`'s Close circle pinned only its glyph and left its fill and edge following the page; the review found it and the entry below fixes it. The mirror list covered the tokens it named, not every call site that owed one.

## 2026-08-29 — M14 track 4 (review fix): the scrims are anchored to the copy in dp, because a fraction of the banner is not where the copy is
- **Scope:** `core/ui/theme/{JellyfinGradients,OverMedia,ChromeBackdrop}.kt`, `core/ui/component/{JellyfinButtons,PillChip,BackdropHeader}.kt`, `feature/home/{HomeHero,HomeScreen}.kt`, `feature/detail/ItemDetailHeader.kt`, `app/{AppScaffold,AppActions}.kt`, `player/ui/UpNextCard.kt`, `core/ui/src/test/.../{ContrastRatioTest,BackdropScrimGeometryTest}.kt`, `app/src/test/.../ChromeGlassTest.kt`, `core/ui/src/androidTest/.../ChromeBackdropTest.kt`, `feature/home/src/test/.../HomeSizingTest.kt`, `design/_shared/{tokens.css,modern.css}`, `docs/features/{theme,design-system}.md`.
- **Entry this supersedes:** the 2026-08-29 entry above. Its doctrine stands; its *geometry* did not, and nine confirmed `/adversarial-review` findings are fixed here.
- **The root finding, and the root fix.** The light ramp was the canvas's own three stops — transparent, .30 at 42%, .78 at the foot — and the new contrast cases pinned the inks at that **foot**. The copy is not at the foot: the compact hero's lockup starts 231dp above it and the compact detail's 228dp, which on their banners is 100–220dp higher up the ramp, at ~0.35–0.60 alpha. Measured on a white backdrop that left the compact eyebrow at ~2.2:1 and the meta at ~3.8:1 — under the 4.5:1 the neighbouring cases demanded, and under what the *dark* ramp gives at the same altitude. A fraction ramp protects a fraction; a lockup is measured in dp and climbs the picture as the font scale grows.
  `BackdropScrim` and `WideHeroScrim` are therefore no longer `Brush`es but `Modifier.backdropScrim(copyZone)` and `Modifier.wideHeroWash(copyEdge)` over the measured size — `heroHalo`'s precedent, which exists in this file for the same class of bug. The vertical ramp climbs out of transparent over 140dp and **holds the .78 plateau from the lockup's own ceiling down**; the wash holds it past the copy column's far edge (24 + 420 + 24dp) and then fades over 280dp. `copyZone` is computed by the layout that draws the copy — `compactHeroCopyZone` from `HomeHero`'s existing sizing constants (231dp at scale 1.0, 386dp at 2.0, shedding with the lockup), `detailLockupCopyZone` from the detail lockup's, both over-measuring on purpose because a taller copy zone only starts the plateau higher.
  **The plateau is the canvas's own .78** — no value moved, only the shape. Both schemes take one plateau, so there is one ground and one set of ratios; the dark `#101010` is the lighter of the two inks and is the case `ContrastRatioTest` now pins (title 9.67, eyebrow 6.65, meta 6.19). The dark ramp keeps a 140dp tail from the plateau to an opaque page, because there the artwork still dissolves.
- **The wash's second flaw was geometric in the same way and in *both* branches** — the dark one had it too, pre-existing. Its .38 mid stop was a fraction of the window while the copy column's right edge is a fixed 444dp, so on every wide window narrower than ~1168dp (the wide layout starts at 600dp) the copy's right portion sat on thinning or transparent wash, and the KDoc and both wide cases were false. It is now dp-anchored and mirrored under RTL.
- **`BackdropScrimGeometryTest`** pins the claim the whole doctrine rests on and no colour test can see: that the plateau is already held at the ceiling, that every altitude below it keeps at least the plateau, that a growing font scale takes the plateau up with the lockup, that a banner with no room starts part-way up the climb rather than jumping, that the dark tail still reaches the page, and that the wash holds past the copy column on the narrowest wide window.
- **Chrome (findings 4, 6).** The ground swap was a one-frame `if` and it fired late: on a tab switch the outgoing screen stays composed for the whole `NAV_TRANSITION_MILLIS` cross-fade, so `onDispose` held the hero's dark glass over the *incoming* page. The report is now gated on the nav entry being `RESUMED` — an entry leaves `RESUMED` when the transition starts — and both consumers cross over a tween off one shared fraction (`chromeGroundProgress`), the band as a true crossfade of its two calibrated ramps rather than a lerp that would be a third ramp, the circles through `ChromeGlass`. `OverMedia.ErrorAccent` moves off the dark scheme's `#CF6679`, which measured **2.15:1** on the chrome fill over a white frame — below 1.4.11's 3:1 and a regression on the light combination it replaced — to `#F0A3AE`, the tint the error banner's dark side already uses, at 3.89:1.
- **Half-fixes closed (findings 5, 7).** `UpNextCard`'s Close circle pinned only its glyph and left fill and edge following the page: in light that is a white@55% disc with a black@10% edge inside a pinned-dark card, glyph ~1.9:1. It now passes the fill and the edge too, and the mirror list pins the call site — which is the correction the entry above needed. `ActionPillChip`'s over-media edge moves from `BadgeBorder` (white@32%, 2.80:1) to `GlassBorder` (white@40%, 3.54:1): a chip is a control, so its edge is its whole boundary. `BadgeBorder` stays the certificate's alone, frozen `Exempt` at 2.46 — an inert label, not a control.
- **`ChromeBackdrop`'s cleared-on-dispose guarantee is now pinned** (finding 8), which it was not: it was asserted in three places and deleting the `onDispose` left `/verify` green. The reporter moved into `:core:ui` as `ReportChromeBackdrop` so there is one implementation, and `ChromeBackdropTest` composes it, toggles it and removes it. **Instrumented, not JVM** — the guarantee is a composition-lifetime one and the unit source sets have no Compose runtime host (no Robolectric; `ui-test-junit4` is wired to `androidTestImplementation` only). It therefore runs on the device as milestone DoD rather than in `/verify`; the alternative was a source-text check, which pins the line and not the behaviour.
- **Tests:** none weakened. Every ratio quoted anywhere in this wave now names the ground it is measured on and has a case that measures that ground; `OverMedia.Accent` gained the case its "every ratio here is pinned" claim owed (3.38:1 on the plateau). `HomeSizingTest` pins the copy zone at both scales, its shedding, and the copy column's edge.
## 2026-08-29 — Settings becomes a category hub with pushed pages, and adopts the navigation-chrome rule ahead of the rest of the app
- **Scope:** `feature/settings/SettingsScreen.kt`, `SettingsHub.kt` (new), `SettingsCategory.kt` (new), `SettingsSummaries.kt` (new), `SettingsCategoryScreens.kt` (new), `SettingsRows.kt`, `feature/settings/src/main/res/values*/strings.xml` (10 new strings × 70 files), `core/ui/component/ScreenHeader.kt`, `core/ui/theme/JellyfinTypeExtras.kt`, `core/common/Routes.kt`, `app/JellyfinNavHost.kt`, `scripts/a11y-scaffolding-allowlist.json`, new unit and instrumented suites, `design/` (the mirrored canvas), `docs/features/settings.md`.
- **Plan said:** `docs/PLAN.md` describes Settings as one surface — "prefs, account, storage location picker, sign out" — reached "behind top-bar avatar", and M9 delivered it as a single flat scroll of six sections. Nothing in the plan anticipates a hub, per-category destinations, or a two-pane tablet shape. The plan's navigation line is "bottom nav bar Home / Libraries / Search / Downloads; Settings behind top-bar avatar", silent on what a pushed screen's header carries.
- **Done instead — the flat scroll becomes a hub plus six pushed pages, per the saved design canvas.** The canvas (`design/screens/settings-hub.html`, `settings-playback.html`, `settings-downloads.html`, `settings-appearance.html`, `settings-landscape.html`, `design/components/settings-rows.html`, `design/foundations/navigation-chrome.html`) is the settled design and CLAUDE.md's design-first rule makes it win over the shipped shape. Concretely:
  1. **The hub is a summary list, not a table of contents.** Each category row carries the category's *current state* — "Skip automatically · Picture-in-picture on", "Wi-Fi only on · 12.3 GB on this device" — which is the only thing that pays for the extra tap. Derivation is pure (`SettingsSummaries.kt`) and returns resource *ids*, never text: a summary resolved into a `String` would carry the build's language rather than the reader's, the `UiText` lesson from audit H8.
  2. **Section headings become the design system's eyebrow** (11sp/W600/0.14em, uppercase in the UI locale, `onSurfaceVariant`) instead of the 17sp primary `SectionTitle`, and rows now sit on 16dp-radius panels with hairlines *inside* a panel and gaps between them. They keep `heading()` semantics and gain a sentence-case `contentDescription`, the `KindHeader` pattern (UI-9).
  3. **Rows inside a category carry no leading icon**; icons identify a category on the hub only.
  4. **Nothing a control does changed.** Every row keeps its string verbatim, its inert inner control, its 48dp target, its `selectableGroup()`, its live region and its confirm dialogs. The storage-location group still hides below two volumes, still announces assertively when the chosen volume is missing, still carries the recovery action hint, and still confirms a switch that would delete downloads.
- **Also done: Settings is the first surface to draw Back as its only leading control.** `ScreenHeader.onHome` becomes `(() -> Unit)? = null`, drawing the Home button only when a caller passes one, so every existing caller (`LicenceScreen`, `ThirdPartyLicencesScreen`, `LibraryGridScreen`, `SyncPlayGroupsScreen`) is byte-identical in behaviour. The rule card describes a system-wide direction — two adjacent glass circles look like one affordance and do different things, and Home is already on the nav bar — but **rolling it out to Library, Detail and Player is deliberately a separate wave**: those screens have their own escape-hatch arguments (Detail can be entered by deep link with no back stack) and changing them here would put an unreviewed nav change inside a settings restructure.
- **The tablet two-pane is width-only at 840dp.** The same `AuthTwoPaneMinWidth` cutoff `ServerSetupScreen` splits at, so the app has one width at which a screen becomes two panes. Unlike `isWideHome` it carries no height or landscape condition: a rail beside a scrolling pane is legible in any 840dp window, and a landscape condition would drop an 840dp portrait tablet to the phone flow for no reason a user could see. The open category is `rememberSaveable` state on the Settings destination, hoisted **above** the width branch — parked inside the two-pane arm it would be remembered by a host that rotating into portrait destroys. `SettingsSizingTest` pins the cutoff, including that the test tablet splits in landscape (1138dp) and pushes in portrait (711dp).
- **New copy, and the one place it does not reuse an existing string.** Ten new translatable strings: two on/off frames (`%1$s on` / `%1$s off` — format strings, not concatenation, so a language can put the state word first), a storage frame, six sub-section eyebrows the canvas introduces ("During playback", "Subtitles", "On this device", "Over the network", "Quality", "Colour"), and `settings_section_network`. The last is the only case where the hub does **not** reuse an existing label: the canvas names the category "Network" while the rows under it were written for a section called "Connectivity", which is still the heading *inside* the page. Two more are `translatable="false"` because neither half is translated: `settings_licence_short` (the SPDX identifier) and `settings_hub_version` (a brand name plus a version).
- **Deliberately not built, though the canvas draws them.** The Account page's *Switch account* and *Manage servers* rows belong to M14 track 1, hard-gated on the server-scoping backend workstream in `docs/notes/m14-multiserver-design-brief.md`; the Account page holds exactly what `AccountSection` held. The Playback mock's dimmed *Gestures* group is placement for M14 track 5, not a spec, and is absent.
- **Two pieces of state upgraded from `remember` to `rememberSaveable`** while they moved: the storage-switch confirmation (`pendingVolumeId`) and the sign-out dialog. Both are dialogs that a rotation previously dismissed; on the tablet two-pane the sign-out dialog is now hosted by the Account *pane*, so switching category dismisses it — which is what leaving the page should do — while a rotation, which does not leave it, keeps it.
- **Reason:** design-first governance (CLAUDE.md) — the canvas is the settled design, and the flat scroll was already the app's longest screen at ~30 rows with ~40 due once M14's remaining tracks land. The growth rule the canvas states ("a category splits at roughly seven rows") is what the hub buys; a seventh section on a flat scroll buys nothing.
- **Tests:** none weakened. New `SettingsSummariesTest` (every summary's exact parts, both switch positions, the API-31 case where the wallpaper row is absent so the summary must not name it, and that every category and pane has a title and a route of its own) and `SettingsSizingTest` (the 840dp cutoff and both test-tablet orientations). `SettingsViewModelTest` and `SettingsRowsTest` are untouched because nothing they pin moved — the ViewModel is unchanged and `choiceRowDescription` is unchanged. `:feature:settings` **leaves** `scripts/a11y-scaffolding-allowlist.json`, having grown the instrumented suite it has owed since 2026-08-05: `SettingsHubA11yTest`, `SettingsCategoryPagesA11yTest` and `SettingsTwoPaneA11yTest` sweep every new screen with ATF at the default scale and at fontScale 2.0, and pin the invariants a restructure is most likely to drop (one-node rows that speak their summary, the inert switch and radio, both skip groups repeating their own name, the info row not being clickable, exactly one Back and no Home). They are written but **not yet run on the device** — `connectedDebugAndroidTest` is milestone DoD and is owed with this wave's device walk.
- **Owed:** a device walk. The two-pane shape, the fontScale-2.0 pass and the light scheme on the Appearance page (the one page drawn light in the canvas, and the one a user opens to change the theme) have been reasoned about but not seen.

## 2026-08-29 — Settings review follow-ups: the storage meter ships without progress semantics, an eyebrow needs a second section, and a category has one name
- **Scope:** `feature/settings/SettingsRows.kt`, `SettingsCategoryScreens.kt`, `SettingsSummaries.kt`, `SettingsCategory.kt`, `feature/settings/src/main/res/values*/strings.xml` (two strings deleted from 70 files), `SettingsSummariesTest`, `SettingsCategoryPagesA11yTest`, `design/screens/settings-appearance.html`, `design/components/settings-rows.html`, `docs/features/settings.md`.
- **Plan said:** nothing about any of this; these are refinements of the 2026-08-29 hub entry above, from the design review of it.
- **Done — (1) the storage row gets its meter back, with no progress semantics at all.** The Downloads mock draws a bar inside the storage info row. The earlier omission argued the info row must stay one merged node; the row does, and the bar does not threaten that, because it is a *visual restatement* of the used/free figures the node already speaks. It carries `clearAndSetSemantics {}` and no `progressBarRangeInfo`, following `MediaCardArtwork.InsetProgressBar`, and `theStorageMeterAddsNoProgressNodeToTheRowItIllustrates` pins the absence so that adding the semantics back reads as the regression it is.
  **This is deliberately the opposite call from `:feature:downloads`' `UsageBar`, which does carry an explicit range** — and the difference is the text beside each. That one is drawn loose, three to a wide layout, with no sentence naming the volume it measures, so an explicit range is the only thing that makes it speak. This one is inside a row whose own text says "12.3 GB used · 41.0 GB free on this device". Both KDocs now name the other so the pair is not "unified" later.
  Height, radius and track ink match `UsageBar` exactly. `storageUsedFraction` duplicates its `usageFraction` arithmetic because features never depend on each other and one division does not earn a `:core:common` seam; the denominator is the whole volume (used + free), and a volume reporting nothing draws an **empty** track, because an unknown size is not a size of zero and a full bar would read as a device out of room.
- **Done — (2) an eyebrow only earns its place on a page carrying more than one section.** On a single-section page the screen title already does that work, and an eyebrow reading "THEME" above a group captioned "Theme" says it twice. Appearance and Network become one panel with no eyebrow. **Applied to Account and About as well**, which the review did not name but which are single-section pages with the same duplication — the alternative was a rule that holds on two pages and not on the two beside them. The choice group's own caption stays: it is the group's a11y anchor, repeated into each row's `contentDescription`, and is not a heading.
- **Done — (3) a category is called the same thing on its hub row and on its own page.** `settings_section_connectivity` is **deleted** from all 70 files, along with the now-unused `settings_eyebrow_colour`; rule (2) removed the last use of both. `aCategoryHasOneNameNotTwo` pins that `SettingsPane.titleRes()` agrees with `SettingsCategory.titleRes` for every category, so a long-form alias cannot be reintroduced quietly.
- **Not done, and flagged rather than guessed: the Appearance artboard's `.caption`.** The updated artboard draws "Android 12 and later only. On older devices the row is absent, not disabled — there is no wallpaper palette behind it." under the wallpaper-colours row. It is not shipped, because it reads as a note to a designer rather than user copy: it describes the row's own absence, which no user who can read it can observe, and the row is only ever drawn on API 31+, so "Android 12 and later only" is trivially true of what is on screen. Every other caption in the artboards maps to an existing shipped string (`settings_quality_supporting`, `settings_storage_location`); this one has none. Adding it means a new string in 70 files, so it is a decision to take rather than infer. The row keeps its supporting text, which is the artboard's `.sb` and is real copy.
- **Reason:** design-first governance; all three are the review's calls on the saved canvas, and the two rules are now written into the canvas's own metrics note.
- **Tests:** none weakened. `SettingsSummariesTest` gains the meter's arithmetic (empty, full, the mocks' 23%, an unknown volume, and clamping against a platform that reports nonsense) and the one-name rule. `SettingsCategoryPagesA11yTest` gains the absent progress node, the eyebrow count on a single-section page (zero) and on a multi-section one, that an eyebrow is spoken sentence-case, and that the theme group keeps its caption with no eyebrow above it. The instrumented suites still have not run on a device.

## 2026-08-29 — M14 track 6: the device walk runs, four checks pass, and two of the remaining three are blocked by the library rather than by the code
- **Scope:** `docs/notes/m14-ass-libass-spike.md` (device checklist rewritten with results), `STATUS.md`. No code changed.
- **Plan said:** the 2026-08-28 track-6 verdict entry lists a seven-check device walk as owed, and says the feature stays experimental until it is walked. It also says: "If (1) or (4) fails, the honest outcome is to downgrade to *do not implement*."
- **Done instead — the walk ran and the checklist is now a result, not a promise.** Debug build of `53e31530` on the test tablet against the dev server. **(1) spacing re-check, (2) embedded MKV, (5) HDR/HEVC, (6) stability: PASS. (3): the styled-sidecar branch passes, the double-merge branch could not be built. (4): not measured, and not constructible as specified. (7): blocked, no receiver.** Details in the note; the parts that are decisions rather than observations:
  1. **(4) is not a failure — it is a check that cannot be run as written, and that is the finding.** It asks for an offset "against server burn-in", but this app has no burn-in path: on a transcode it delivers subtitles as in-manifest VTT by design (the 2026-08-21 drift fix), so ASS never reaches the client there. There is nothing in-app to measure against. Measuring against the source file instead was attempted and defeated by content, not method: every ASS-bearing title in this library is near-static dialogue (mean inter-frame |Δ| ≈ 0.6 of 255 over the matching region), so frame-matching a screen recording back to the source is degenerate — many captured frames match one source frame — and onset cannot be resolved to better than several frames. No gross (~0.5 s) lateness was visible, which is worth something and is *not* a pass. **The verdict entry's "if (4) fails" clause is therefore not triggered; (4) is still open.**
  2. **(3)'s guard branch is unreachable from this library, and that is a content gap, not a code doubt.** Audio sidecars are planned only when `quality.isTranscoded`, and `DownloadEnqueuer.planQuality` stamps `ORIGINAL` on any transcode that would not save 10 % (`ORIGINAL_THRESHOLD = 0.9`). Every multi-audio ASS item here is low-bitrate enough to be downgraded — a 2.3 Mbps x265 episode estimates *larger* at `LOW` (475 MB against 379 MB, logged verbatim by the enqueuer) — and the single item that does transcode has one audio track. Closing it needs a multi-audio ASS source above ~4 Mbps. Until then `styledAssSurvivesMerge` is pinned only by `AssMergeCompatTest`.
  3. **New finding, and it belongs in the feature's copy eventually: a transcoded download loses the MKV's font attachments.** The server's output carries video and audio only (`ffprobe` on the downloaded file's header: `h264`, `aac`, no attachment streams), and the sidecar is a bare `.ass`, so libass has no embedded font provider to consult and logs `fontselect: Using default font family: (AG Foreigner-Roman, 700, 0) -> /system/fonts/Roboto-Regular.ttf, Roboto-Bold`. Positioning, colour, scale, blur and fades survive; the typeface does not. An `ORIGINAL` download keeps the whole MKV and therefore keeps its fonts. This is a fourth "when it steps aside" case, weaker than the other three (it degrades one property rather than the whole feature), and it is recorded here rather than shipped as copy because the feature is still default-off; it should reach `playback.md`'s table when the switch graduates.
  4. **`AssFontConfig` is confirmed on hardware, twice over.** No *"No usable fontconfig configuration file found"* line appears in any capture, `Using font provider fontconfig` appears in every session, words are spaced, and the attachment-less fallback above lands on Roboto-Bold rather than an arbitrary face — which is precisely what the `sans-serif` alias was added to guarantee. The 2026-08-28 device fix is verified.
  5. **Karaoke is untested and untestable here.** No `\k`/`\kf`/`\ko` tag exists in any ASS track in this library, so check (2)'s "karaoke" clause was dropped rather than claimed; "animation" is covered only as `\fad`/`\blur`/`\fscx`, not `\t`.
  6. **A method note that cost this walk two passes, now written into the checklist as step (0):** `uiautomator`'s `checked=` on the settings row is not a reliable read of the preference — it was misread twice and sent a whole pass down the wrong branch. The authority is `files/datastore/app_preferences.preferences_pb`, where `styled_ass_subtitles` is followed by `12 02 08 01` for on. And because the preference is read once per player build, a relaunch that reuses the process keeps the old player and the old answer, so every A/B needs a `force-stop` between toggles.
- **Reason:** governance rule 3 and the audit's "every doc claim needs a test" at documentation scale — a checklist that says "nothing below has run on hardware" while four of its items have is exactly the doc-describing-code-that-does-not-exist class, in reverse. Recording (4) as unconstructible is the load-bearing part: leaving it as "owed" would have kept a check on the list that no amount of walking can discharge.
- **Not done:** no code changed, no default flipped, no copy moved. The switch stays default-off and the feature stays experimental; graduation still needs (3)'s guard branch, (4) and (7).
- **Tests:** none touched, none weakened (docs-only). The walk exercised `AssMergeCompatTest`'s subject only on its passing branch; the guard branch remains unit-pinned.

## 2026-08-29 — M14 track 6 (device fix): a transcoded download carries the container's fonts, and they go to `Ass` rather than to `AssHandler`
- **Scope:** `core/common/model/DownloadFileType.kt` (`FONT`), `data/downloads/plan/DownloadUrlFactory.kt` (`attachmentUrl`), `data/downloads/plan/DownloadFilePlanner.kt` (`fonts()`), `data/downloads/offline/DownloadedMedia.kt` (`DownloadedFont`), `data/downloads/offline/DownloadedMediaProvider.kt`, `data/downloads/engine/SubtitleSidecarTopUp.kt`, `player/model/PlaybackMediaSource.kt` (`LocalFont`), `player/model/PlaybackMediaItemSpec.kt` (`FontSpec`), `player/resolve/LocalPlaybackResolver.kt`, `player/resolve/ExoMediaSourceFactory.kt`, `player/session/AssSubtitleSupport.kt` (`addFonts`), `player/session/ExoPlayerHandle.kt`, `scripts/pattern-baseline.json`, tests, `docs/features/playback.md`, `docs/notes/m14-ass-libass-spike.md`, `STATUS.md`.
- **Plan said:** the 2026-08-28 verdict entry lists the integration as *wrapping* Media3 and records the ways the feature "steps aside" — a libass load failure, the double-merge shape, transcodes, casting. None of them is "the faces are missing", because nothing had walked a downloaded transcode yet.
- **Done instead — the walk's one real defect is fixed rather than documented.** The 2026-08-29 walk found that a transcoded download draws styled ASS in the fallback family: the server's re-encode carries video and audio only (`ffprobe` on the downloaded file's header: `h264`, `aac`, no attachment streams), the sidecar is a bare `.ass`, and libass logged `fontselect: Using default font family: (AG Foreigner-Roman, 700, 0) -> /system/fonts/Roboto-Regular.ttf, Roboto-Bold`. Positioning, colour, scale, blur and fades all survived; only the typeface was wrong.
  1. **The plan fetches the attachments.** A new `DownloadFileType.FONT`, one file per `MediaAttachment`, through `VideoAttachmentsApi.getAttachmentUrl`. Three filters, each of which is the difference between a useful file and wasted bytes: the row's **quality** must be a transcode, exactly as `audioSidecars` is guarded and for the same reason (an `ORIGINAL` download *is* the container, so `withAssMkvSupport` reads the faces out of the file already on disk); the plan must contain an **ASS/SSA** sidecar (a font is only ever reached through an ASS style); and the attachment must look like a font by **mime type or extension** (Matroska carries cover art and credits too, and older servers say `application/octet-stream`). One with no file name is skipped for want of something to name the stored file after — not, as this entry first said, for want of a lookup key, which is the face's own family names.
  2. **`addFonts` goes to `handler.ass`, not to `AssHandler.addFont`, and that is the whole fix.** The obvious call does not work, and the device said so before the reasoning did: with 13 fonts registered, every style still resolved to `Roboto-Bold`. `AssHandler.addFont` hands a face to libass only when the handler already has tracks and parks it in `pendingFonts` otherwise; that list is drained by `createTrack` — *after* its first statement, `createRenderIfNeeded()`. libass builds its font lookup exactly once, inside the `ass_set_fonts` call that creating the renderer makes, so a face parked before the first track arrives one step too late and is ignored for the whole session. `Ass.addFont` is the same native call without the detour. Verified by disassembling `AssHandler` (`javap -c`), then re-walked: the same item now logs `fontselect: (AG Foreigner-Roman, 700, 0) -> AGForeignerRomanMedium` and draws in the attached face.
  3. **The read is blocking, deliberately.** The bytes must be in the library before the first `createTrack`; posting them to another thread would reopen by luck the race (2) closes by construction. Tens of KB per face, on a path already opening the media file.
  4. **The top-up is widened from `SUBTITLE` to `SUBTITLE + FONT`, which is a decision the old test explicitly demanded be one.** `SubtitleSidecarTopUpTest` carried the comment "the `type == SUBTITLE` filter is the whole guard, and widening it has to be a decision"; that comment now names the widening and its date. Fonts qualify on exactly the grounds audio does not: tens of KB rather than ~165 MB, and useless without the subtitle beside them. It makes every download taken before this shipped repair itself on the next connectivity edge — confirmed on device: 13 faces appeared, including the one that had been falling back.
- **Reason:** the walk exists to find things the tests cannot, and this is one — no unit test can see that a server strips attachments, and none can see that an upstream method defers a call past the point where it would matter. Documenting the gap and shipping it would have left the feature's headline case (a downloaded anime with styled signs) drawing in the wrong face.
- **Deliberately not done:** no `clearFont` between items. `AssHandler` outlives the item and dropping the accumulated set would also drop attachments an extractor added for a container playing right now; the bound is one video session's items at tens of KB a face, and a name collision resolves to the same face anyway.
- **The ratchet baseline moves by one, and this is the explanation the rule asks for.** `DownloadedMedia.kt` goes from 4 public top-level declarations to 5: `DownloadedFont` joins `DownloadedMedia`, `DownloadedSubtitle`, `DownloadedAudio` and `DownloadedTrickplay`, which are public for the same single reason — `:player` consumes this file as the hand-off out of the download pipeline. Splitting it into its own file would have satisfied the per-file count while scattering one model family, which is working around the gate rather than answering it.
- **Tests:** none weakened. New: eight `DownloadFilePlannerTest` cases (a transcode fetches them; `ORIGINAL` does not; a SubRip-only sidecar set does not; cover art and credits are left behind; an `application/octet-stream` font is caught by its extension; a `../..` file name cannot escape the item directory; a nameless attachment is skipped; no font is essential), two `SubtitleSidecarTopUpTest` cases (a pre-existing download gains its faces; one already on disk is not refetched), three `DownloadedMediaProviderTest` cases (carried with their names; a vanished file is dropped; none reported when there are none), two `LocalPlaybackResolverTest` cases and two `ExoMediaSourceFactoryTest` cases (they reach the spec and take no merge child; a streamed source carries none). The device re-walk is the evidence for (2), which no local test can reach.

## 2026-08-29 — M14 track 6 (adversarial review): the font branch gains the quality guard its own prose claimed it did not need
- **Scope:** `data/downloads/plan/DownloadFilePlanner.kt` (`fonts()` gains a `quality` parameter and guard), `core/common/model/DownloadFileType.kt` (KDoc), `data/downloads/plan/DownloadUrlFactory.kt` (KDoc), `player/model/PlaybackMediaItemSpec.kt` (`FontSpec` KDoc), `data/downloads/plan/DownloadFilePlannerFontsTest.kt` (new, the eight font cases moved out plus one), `DownloadFilePlannerTest.kt`, `docs/features/playback.md`, `DECISIONS.md`.
- **Plan said:** the 2026-08-29 fix entry, `fonts()`'s KDoc, `DownloadFileType.FONT`'s KDoc, `DownloadUrlFactory.attachmentUrl`'s KDoc and `docs/features/playback.md` all justified the *absence* of a quality guard the same way: "an `ORIGINAL` plan has no sidecars at all, so the case falls out for free".
- **Done instead — the premise was false, and three of the five review lenses found it independently.** `subtitles()` plans a sidecar when `stream.isExternal || quality.isTranscoded`. The quality half is only about *embedded* streams; an **external** subtitle — a `.ass` file sitting beside the video on the server, which is the ordinary shape in the libraries this feature targets — is planned at *every* quality, and the repo already had a test pinning exactly that. So an `ORIGINAL` download of such an item reached `fonts()` with a styled sidecar in hand and an untranscoded container behind it, and downloaded every attached face a second time: bytes already inside the `.mkv` the same plan was storing whole, then surfaced by the provider, carried into the spec, and blocking-read at every `prepare`. No rendering defect — `withAssMkvSupport` reads the container's own copies and a name collision resolves to the same face — but redundant network, redundant storage and redundant work, in flat contradiction of four documented claims.
  1. **`fonts()` is now guarded on `quality.isTranscoded`, exactly as `audioSidecars()` is and for the same reason**, and the KDoc says why it has to be its own condition rather than a corollary of the sidecar one.
  2. **The test that "pinned" the ORIGINAL case was the one that hid it.** It passed `external = false`, the single value for which the old claim held. It keeps its case and gains the sibling — `external = true` at `ORIGINAL`, asserting the sidecar *is* planned and the fonts are *not*.
  3. **Two KDocs said the opposite of the code and are corrected.** `FontSpec.path` named `AssHandler.addFont` — the exact call this feature exists to avoid, so a maintainer following it would have reintroduced the silent fallback the fix was written for. And `fonts()`'s third filter claimed a missing `fileName` makes an attachment unaddressable "because libass matches on it", which contradicts `sanitisedFileName`'s KDoc four lines below saying (correctly) that the lookup key is the face's own family names; the skip is real but its reason is "nothing to name the stored file after".
- **Reason:** governance rule 4-adjacent and the audit's "fix the sibling too" — `subtitles()`'s two-part condition *is* the sibling, four functions above the new code, and the wave read only half of it. This is the class the checklist names as the most recidivist in the repo, and the gate caught it doing exactly what it was written to catch.
- **Also done, to keep detekt honest without moving a threshold:** the eight font cases move to their own `DownloadFilePlannerFontsTest`, because adding the ninth pushed the parent class past `LargeClass`. Splitting a cohesive new branch out is the answer to that rule; raising it would not be. `FakeDownloadUrlFactory` becomes `internal` so both classes share one fake. `fonts()` itself carries a targeted `@Suppress("LongParameterList")`, matching `plan()` and `media()` beside it.
- **Reviewed and deliberately not changed** (raised by the gate, refuted on verification, recorded so they are not re-raised): the blocking `File.readBytes()` on `prepare` and the never-cleared font set are both recorded trade-offs in the entry above, and the "registered %d of %d" log cannot count faces libass accepted because `Ass.addFont` returns `void`.
- **Review result:** 5 confirmed of 15 raised across 5 lenses, all low severity; three were this one defect seen from three angles, two were the KDoc contradictions. 10 refuted.
- **Tests:** none weakened. `DownloadFilePlannerFontsTest` holds nine cases; the new one is the external-sidecar-at-ORIGINAL sibling.

## 2026-08-29 — subtitle appearance: size and background for the subtitles the app draws itself, defaulting to the device
- **Scope:** `core/common/model/SubtitleAppearance.kt` (new), `core/datastore` (`AppPreferences`, `DataStoreAppPreferences`, `PreferenceKeys`), `player/ui/SubtitleAppearanceStyle.kt` (new), `player/ui/PlayerScreen.kt`, `player/ui/PlayerUiState.kt`, `player/ui/PlayerViewModel.kt`, `feature/settings` (`SettingsUiState`, `SettingsViewModel`, `SettingsScreen`, `SettingsCategoryScreens`, `SettingsRows`, `SettingsSummaries`), 70 `strings.xml`, tests, `docs/features/playback.md`, `docs/features/settings.md`.
- **Plan said:** nothing. `docs/PLAN.md`'s M9 settings scope lists skip modes, PiP, download quality/Wi-Fi, storage and theme; M14 track 6 is the ASS/SSA spike. Subtitle *appearance* — how large the app draws a subtitle and what it draws behind it — appears nowhere, for any format.
- **Done instead — user-requested scope addition, built on the established row idiom.** Two preferences in Settings → Playback → Subtitles, above the styled-ASS switch: **Text size** (Follow the device / Small / Normal / Large / Larger) and **Background** (Follow the device / None / Translucent / Solid).
  1. **Both default to `SYSTEM`, and that is the load-bearing decision.** `PlayerView` untouched calls `setUserDefaultTextSize()` and `setUserDefaultStyle()`, which read Android's `CaptioningManager`. Any other default would silently override a caption size a user set in accessibility settings and would change how every existing install looks on upgrade. `SYSTEM` is not "no preference" — it is the explicit choice to defer, and the two `null`s that carry it are what route back to those two Media3 calls.
  2. **Applied through two independent calls, not one style object**, so size can follow the device while the background is overridden or the reverse. `NORMAL` is deliberately `SubtitleView.DEFAULT_TEXT_SIZE_FRACTION` (0.0533): the option a user reads as "normal" has to be what the player would draw with no preference at all.
  3. **The outline rides with the background rather than being its own setting.** It is only load-bearing when there is no box — white-on-white is unreadable — so `NONE` is outlined and the other two are not. Offering it separately would let a user build the one combination that cannot be read.
  4. **Collected, not read once — the opposite of the styled-ASS switch beside it.** These reach `PlayerUiState` and are applied in the `AndroidView`'s `update`, so a change made in Settings redraws the cue on the playback already on screen. That is possible here and not for libass because nothing about the renderers, extractors or parser depends on them; DECISIONS 2026-08-28 records why the ASS toggle cannot do the same.
  5. **Media3's renderer only, and the copy says so.** With styled ASS on, an ASS/SSA cue is parsed by `AssNoOpSubtitleParser` and drawn by libass from the script's own styles, so `SubtitleView` has no cue of that track to draw and these settings have nothing to act on. Every other format — and ASS/SSA whenever that switch is off — is drawn here. The group's supporting line states it rather than leaving a user to discover that one format ignores the size they chose.
- **Reason:** the request. It is also the gap the styled-ASS A/B made visible on device: with libass off, an SSA line falls back to the platform caption style — an opaque black box — and nothing in the app could change it.
- **Design-first rule, and why this is exempt:** CLAUDE.md sends new UI through Claude Design. This adds no visual pattern — two `SettingsChoiceGroup`s of `SettingsChoiceRow`s, the same components the skip modes, download quality and theme already draw, in an existing section of an existing page. The one component change is an optional `supportingText` on `SettingsChoiceGroup`, defaulting to `null` so every existing caller is untouched. A **subtitle preview** would be new UI and is deliberately not built here; it is the obvious follow-up and belongs on a canvas first.
- **Not done:** no per-item override, no colour, no font, no position, no edge style beyond the outline that comes with `NONE`, and nothing in the player's own subtitle sheet — the picker there still only chooses a *track*. The playback summary row is unchanged: both settings default to following the device, and a summary naming a setting nobody had touched would crowd out the two that say something (its KDoc, which said "four preferences", now says six and why).
- **Tests:** none weakened. New: `SubtitleAppearanceStyleTest` (SYSTEM is the only deferring option for both; both defaults are SYSTEM; the sizes are a strictly increasing ramp in declaration order, which is the order the picker draws; `NORMAL` equals Media3's own default fraction; the background ramp runs clear → opaque and only the clear one is outlined; an unknown stored name decodes to SYSTEM). `DataStoreAppPreferencesTest` gains the two defaults, a round trip, an unreadable-name decode and an emission case. `SettingsViewModelTest` gains the write-through for both and the follow-the-device projection. `PlayerViewModelTest` gains the default and — the one that matters — that a change mid-playback reaches the item on screen. `SettingsCategoryPagesA11yTest` gains two cases: "Follow the device" now appears twice on the Playback page, so each row's description must repeat its own group name, and the new group caption is drawn without being spoken. `validate_i18n.py` green across 759 locale files.
## 2026-08-29 — Settings owns the Wi-Fi-only switch outright; the Downloads screen answers "why is nothing moving" instead
- **Scope:** `core/network/connectivity/ConnectivityMonitor.kt`, `AndroidConnectivityMonitor.kt`, `data/downloads/DownloadRepository.kt`, `impl/DownloadRepositoryImpl.kt`, `engine/MeteredConnection.kt` (KDoc only), `feature/downloads/DownloadsUiState.kt`, `DownloadsViewModel.kt`, `DownloadsScreen.kt`, `feature/downloads/src/main/res/values*/strings.xml` (three strings added and three removed across 70 files), `DownloadsUiStateTest`, `DownloadsViewModelTest`, `DownloadRepositoryImplTest`, `ConnectionStateProviderTest`, `docs/features/downloads.md`, `docs/features/offline-read.md`, `docs/ARCHITECTURE.md`.
- **Plan said:** `docs/PLAN.md` names the preference only as an *input* — M7's enqueue step takes "constraints from prefs (UNMETERED if Wi-Fi-only…)" and the M7 DoD is "Wi-Fi-only honored". It says nothing about where the control the user flips lives, and nothing about the Downloads screen's chrome beyond the pipeline it drives. So the plan is silent, and this is logged anyway: it is a deliberate reversal of shipped behaviour, removing a control the user could reach from the Downloads tab since M7.
- **Done instead — the switch leaves the Downloads screen entirely, and a conditional notice replaces it.**
  1. **`WifiOnlyToggle` and `NetworkStatPanel` are deleted.** Both wrote `AppPreferences.downloadOverWifiOnly`, and so does Settings › Downloads' own row: one key, two controls, which is a duplicated *control*, not a state bug. `StorageCard` becomes storage-only and `WideSummary` drops from three equal panels to two. Settings is the sole owner.
  2. **`WaitingForWifiNotice` appears only when the preference is *currently* the reason nothing is moving** — `wifiOnly && onMeteredNetwork && wifiBlockedCount > 0`, derived on `DownloadsUiState` as constructor-body `val`s like everything else there (the queue emits two to six times a second; a `get()` would re-walk it per read). The rest of the time the screen carries no preference at all, which is the point: a setting nobody is currently blocked by is Settings' business.
  3. **`wifiBlockedCount` counts `QUEUED` and `DOWNLOADING` and nothing else.** `DownloadStatus`' own KDoc is the authority: `PAUSED` is a user decision that survives process death, and a download stopped by leaving Wi-Fi goes back to `QUEUED` so the constraint resumes it. A user-paused or errored row is not waiting for a network, and letting it raise the notice would make the app blame the connection for the user's own tap.
  4. **`DownloadsChromeState.wifiOnly` becomes `queuePausedForWifi`** — the derived verdict, not the raw preference. The chrome's stated rule (every field a scalar or a value type from this module) is unchanged; it simply no longer needs to know what the preference says, only what follows from it.
- **`ConnectivityMonitor` gains `isMetered`, from the *same* registration.** `AndroidConnectivityMonitor` now derives both flows from one private shared `Flow<NetworkFacts>`: `NetworkCapabilities` answers "has internet" and "is metered" in one object, so a second `registerDefaultNetworkCallback` would be a second system registration for a fact the first one already has. The `NET_CAPABILITY_INTERNET`-not-`VALIDATED` choice, its LAN-Jellyfin rationale, the null-`ConnectivityManager` fallback and the `distinctUntilChanged().conflate().shareIn(WhileSubscribed(replayExpiration = 0), replay = 1)` treatment are all preserved verbatim on the shared upstream.
  **`isMetered` is `false` when there is no network at all**, and is gated on the same `NET_CAPABILITY_INTERNET` `hasNetwork` reads, so the two can never disagree about whether a transport counts. "Not on a metered network" is the honest answer when there is no network to be metered; a notice saying "waiting for Wi-Fi" at an offline device would be a lie, and that device already has the offline banner. The null-manager fallback is `false` here — matching `AndroidMeteredConnection`'s existing "no connectivity service reads as unmetered" rule — while `hasNetwork`'s stays `true`.
- **Routed through `:data:downloads`, not a new module edge.** `feature/downloads/build.gradle.kts` carries a deliberate "no `:core:network` edge" comment; it still does. `DownloadRepository` gains `onMeteredNetwork: Flow<Boolean>`, delegated in the impl to the injected `ConnectivityMonitor.isMetered`, and the ViewModel's projection `combine` takes it as a fourth source.
- **The metered-connection sibling is documented in both directions rather than unified.** `engine/MeteredConnection` stays exactly as it was — an `internal`, synchronous, one-shot answer for `SubtitleSidecarTopUp`, which runs on the application scope outside the worker's `UNMETERED` constraint and must decide once before it starts. Its KDoc and `DownloadRepository.onMeteredNetwork`'s now each name the other and say why both exist: collecting a flow for a single boolean would cost a coroutine and a subscription, and polling a one-shot on a timer would give a UI that lags the radio. Undocumented, this is exactly the duplicated-sibling class the audit checklist names.
- **Copy.** Three new strings in all 70 files (`downloads_wifi_wait_title` / `_body` / `_action`), three removed from all 70 (`downloads_wifi_only`, `downloads_stat_network`, `downloads_stat_network_helper` — each confirmed unreferenced first). The action reads *Turn off Wi-Fi only*, naming the Settings row verbatim: it flips a **persistent** preference, so it must not read as a one-time exception ("Use cellular just now"), and it must name the thing the user will later look for to undo it.
- **Accessibility.** Title and body are one merged node (`semantics(mergeDescendants = true)`) carrying `LiveRegionMode.Polite`; the `TextButton` deliberately stays outside that merge as its own focusable action, and carries `Dimens.MinTouchTarget`. **Polite, not Assertive**, and the code says so where the reader will ask: `SettingsCategoryScreens`' missing-storage-volume warning a few files away is Assertive, and the difference is that a vanished volume is losing data right now while a metered connection is merely waiting. The notice is not `:core:ui`'s `ErrorBanner` — red and failure-shaped for a state that resolves by itself is the wrong register.
- **Drawn in both layout branches.** The notice sits inside `DownloadsChrome`, above `DownloadsTabRow`, on the one code path both the compact and the wide arm run through — the sibling-branch trap this screen has already paid for once.
- **Reason:** two controls over one key meant the Downloads screen owned a setting it had no reason to own, and the screen's own failure mode — a queue that sits still and does not say why — went unexplained by the very control sitting above it. Moving the switch to its one home and replacing it with a conditional explanation costs the user nothing (the notice's action reaches the same preference) and makes the stall self-explaining.
- **Tests:** none weakened. `DownloadsUiStateTest` gains a `queuePausedForWifi` truth table (on + metered + queued → true; in-flight counts; empty queue → false; a queue of only `PAUSED`/`ERROR` → false; unmetered → false; preference off → false) and pins `wifiBlockedCount`'s status set directly against the whole `DownloadStatus` enum, so adding a status silently to the count fails. `DownloadsViewModelTest`'s `chrome.wifiOnly` assertion was **updated** to `chrome.queuePausedForWifi` (its subject legitimately changed), and it gains the metered signal reaching the state and the notice clearing itself when the network un-meters. `DownloadRepositoryImplTest` pins the delegation both statically and as the monitor moves.
- **The notice's action stacks under its text, rather than sitting beside it (orchestrator review).** The first cut put icon, text and `TextButton` in one `Row` with the text column weighted. The button is not weighted, so it takes its intrinsic width first and the message gets what is left — and the message loses that race twice over. On a 390dp compact layout the inner width after `PanelPadding`, `SpaceLarge`, the icon and two gaps is about 274dp, while the English *Turn off Wi-Fi only* at fontScale 2.0 wants roughly 290dp on its own; and several translations are half again as long as the English (`values-tr`'s *Yalnızca Wi-Fi seçeneğini kapat* is 31 characters), which crosses the budget nearer fontScale 1.2. A fontScale breakpoint in the shape of `cardTitleMaxLines` would have been the local idiom, but every new locale re-opens the question it answers; stacking removes the contest instead. The icon is `Alignment.Top` rather than centred for the same reason — at large scales the body wraps to several lines and a centred icon floats off the title it belongs to. `:feature:downloads` has no instrumented suite, so nothing would have caught this on a gate.
- **`AndroidConnectivityMonitor` is unit-tested without Robolectric.** The first pass left it untested on the grounds that `:core:network` has no Robolectric and adding one is a decision to take rather than infer — true, but the premise was wrong: `LicenceViewModelTest` already mocks `android.content.Context` and `Resources` with MockK in a plain JVM unit test, so the framework classes are mockable here and no module needs to grow a Robolectric dependency. `AndroidConnectivityMonitorTest` mocks `ConnectivityManager`, captures the registered `NetworkCallback` and drives it: 8 cases covering the unmetered and metered seeds, an absent network, a transport with capabilities but no `NET_CAPABILITY_INTERNET`, a Wi-Fi→cellular transition that must flip metered *without* dropping the network, `onLost`, the null-`ConnectivityManager` fallback, and — pinning the KDoc's own claim — that collecting **both** flows costs exactly one `registerDefaultNetworkCallback`. Checked against a mutant: dropping the `hasInternet` gate from `isMetered` fails the no-internet case and nothing else, so the suite is not passing vacuously.
- **Owed:** `:feature:downloads` has no `androidTest` source set (it is on `scripts/a11y-scaffolding-allowlist.json`), so the notice's polite live region and 48dp target are pinned by code review and nothing else — `config/lint/lint.xml` disclaims Compose semantics outright and does not carry `TouchTargetSizeCheck`, so `:app:lintDebug` sees neither of them. The instrumented case belongs with the suite that allowlist entry owes. A device walk of the notice at fontScale 2.0 is unrun.

## 2026-08-29 — M14 (adversarial review, subtitle-appearance wave): the blocking font read comes off the main thread, and the caveat nobody could hear starts speaking
- **Scope:** `player/model/PlaybackMediaItemSpec.kt` (`FontSpec` carries bytes, not a path), `player/resolve/ExoMediaSourceFactory.kt` (reads them), `player/session/PlaybackSessionController.kt` (builds the spec on IO; `ioDispatcher` is a required constructor parameter), `player/session/AssSubtitleSupport.kt` (KDoc), `feature/settings/SettingsRows.kt` (`SettingsChoiceGroup`'s caption is no longer silenced), `SettingsCategoryPagesA11yTest.kt`, `ExoMediaSourceFactoryTest.kt`, `PlaybackSessionControllerTest.kt`, `DownloadFilePlannerFontsTest.kt`, `PlayerViewModelFixture.kt`, `PlayerViewModelCastTest.kt`, `STATUS.md`, `DECISIONS.md`.
- **Plan said:** two claims this repo had already written down and defended. The entry above records the blocking `File.readBytes()` inside `prepare` as **raised by the previous gate and refuted on verification** — "a recorded trade-off". And `SettingsChoiceGroup`'s KDoc said its `supportingText` "is silenced … and stays a visual aid".
- **Done instead — both were wrong, and the second review found them:**
  1. **The font bytes are read where the spec is built, not where it is prepared.** The earlier refutation conflated *reading the bytes* with *calling `Ass.addFont`*. Only the second has to happen on the player's looper before the first `createTrack`; the first is ordinary file I/O that was landing on the main thread because `PlaybackSessionController.open` runs there (it must — `prepare` binds Media3 to this looper). An item with 13 attached faces — the repo's own device-walk item — therefore opened and fully read 13 files on the UI thread at playback start, again on every `reopen` and every next-episode. `ExoMediaSourceFactory` now reads them while building the spec, `open` calls it inside `withContext(ioDispatcher)`, and `FontSpec` carries the bytes. `addFonts` is unchanged in what it guarantees and now does no I/O at all, so **the ordering fix that this whole feature turns on is untouched**: the native calls still happen inside `prepare`, before any track exists.
  2. **`ioDispatcher` has no default, unlike `mainDispatcher`.** The main one defaults because a plain-JVM test cannot resolve `Dispatchers.Main`; no such excuse exists here, and a default is exactly how a caller would silently lose the hop that keeps those reads off the UI thread. Five test constructions name a test dispatcher instead.
  3. **The subtitle-appearance caveat is spoken.** *Applies to subtitles Jellyboost draws. Styled ASS subtitles keep their own.* is the only place in the app that says the two new preferences do not touch a libass-drawn track — and it was `clearAndSetSemantics {}`, so it existed in no semantics node at all. The group's KDoc had even written the rule it was breaking ("a group whose caveat a screen-reader user needs must put it in the rows' own descriptions instead"). The two options are both worse than the third: folding it into the five rows says it five times, and silencing it loses it. The caption now gets **one traversal stop of its own**; the group *label* stays silenced, because every row does repeat that. The KDoc now states the distinction rather than one half of it.
  4. **The test that pinned (3) could not fail.** `onNodeWithContentDescription` reads `SemanticsProperties.ContentDescription`; a `Text` publishes `SemanticsProperties.Text`. The assertion held whether or not the node was cleared, and the "is drawn" half of its own name was never asserted. It now asserts the caption is present as text exactly once and appears in no row's description — each half failing under the opposite mistake.
  5. **`sanitisedFileName`'s two guarantees get the tests its language twin already had.** `takeLast` rather than `take` (so a 500-character name keeps its extension) and the `?: "font"` fallback were both unpinned while `sidecarLanguage`'s identical pair was pinned four functions above. The recidivist class again, in the test file this time.
- **Reason:** the wave's own `/adversarial-review`, which had not run before it reached `main` — it went out under "merge and push" while the gate was still owed. It raised 9, all 9 survived a refute pass, and the two substantive ones are above. A refute pass that kills nothing is itself suspect, so each was re-checked by hand against the code before being acted on.
- **Recorded, not fixed — `d08a3304` does not compile standing alone.** Its own fake gained `override val isMetered`, a member `ConnectivityMonitor` did not have until `273317a8` on the parallel `m14/downloads-wifi-notice` branch, which is not an ancestor of it (`git merge-base --is-ancestor` says no). The commit was verified in a worktree that already carried the other branch's work — the shared-main-tree hazard, in the very commit whose message warns about trusting a green line. `dad3ea67` merges the branch and every commit from there on is fine; only a bisect landing exactly on `d08a3304` or `bf9882d3` sees it. Rewriting pushed history to repair a two-commit window is the larger harm, so it stays and is written down here instead.
- **`scripts/pattern-baseline.json`'s `SubtitleAppearance.kt` line was added a commit late.** Governance rule 6 wants the baseline move in the same commit as the file it exempts; `bf9882d3` added the file and `d08a3304` added the line. The justification is sound and was given (all 16 siblings under `core/common/model` carry the same count — a shared model is public because it is shared), but it was given under a build-fix subject. Named here so the deliberate act has a record where the rule expects to find one.
- **Tests:** none weakened. New: `the spec is built off the calling thread, because that is where the fonts are read` (a recording dispatcher proves the hop happens and that it completes *before* `prepare`), `a font whose file is gone is dropped, and the rest still reach the spec`, `an overlong font file name is bounded from the tail, so the extension survives`, `a font file name that sanitises to nothing falls back rather than ending in a dot`. Changed subjects: `downloaded fonts are read into the spec` now writes real files to a `@TempDir` and asserts bytes rather than paths (its subject legitimately changed); the a11y caption case is rewritten as described in (4) and renamed to what it now checks.

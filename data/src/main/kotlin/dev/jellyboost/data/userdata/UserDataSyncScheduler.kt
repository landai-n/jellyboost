...
/** Behind an interface so the local-first write path is unit-testable without WorkManager. */
internal interface UserDataSyncScheduler {
    /** Unique work: a second call while one is pending is a no-op. */
    fun enqueue()
}
...
internal class WorkManagerUserDataSyncScheduler
    ...
    constructor(
        ...
        override fun enqueue() {
            ...
            // failed toggles must not push the one scheduled run further into the future.
            runCatching {
                WorkManager
                    .getInstance(context)
                    .enqueueUniqueWork(UserDataSyncWorker.UNIQUE_WORK_NAME, ExistingWorkPolicy.KEEP, request)
            }.onFailure {
                // Losing the retry must not break the local write that triggered it — the row keeps
                // `toBeSynced = true`.

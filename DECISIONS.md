...
- **Owed:** `AndroidConnectivityMonitor` still has no unit test — `:core:network` has no Robolectric and the class is a `registerDefaultNetworkCallback` wrapper, so testing it here would mean adding a test dependency to the module, which is a decision to take rather than infer. `:feature:downloads` has no `androidTest` source set (it is on `scripts/a11y-scaffolding-allowlist.json`), so the notice's polite live region and 48dp target are pinned by code review and the `:app:lintDebug` gate only; the instrumented case belongs with the suite that allowlist entry owes. A device walk of the notice at fontScale 2.0 is unrun.
